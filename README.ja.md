...
# パッケージマネージャー
npm i -g openresearch-ai@latest        # bun/pnpm/yarn でもOK
scoop install openresearch             # Windows
choco install openresearch             # Windows
brew install anomalyco/tap/openresearch # macOS と Linux（推奨。常に最新）
brew install openresearch              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S openresearch            # Arch Linux (Stable)
paru -S openresearch-bin               # Arch Linux (Latest from AUR)
mise use -g openresearch               # どのOSでも
nix run nixpkgs#openresearch           # または github:anomalyco/openresearch で最新 dev ブランチ
```
...
OpenResearch はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/openresearch/releases) から直接ダウンロードするか、[openresearch.ai/download](https://openresearch.ai/download) を利用してください。
...
1. `$OPENRESEARCH_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.openresearch/bin` - デフォルトのフォールバック
...
OpenResearch には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://openresearch.ai/docs/agents) の詳細はこちら。

### ドキュメント

OpenResearch の設定については [**ドキュメント**](https://openresearch.ai/docs) を参照してください。

### コントリビュート

OpenResearch に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### OpenResearch の上に構築する

OpenResearch に関連するプロジェクトで、名前に "openresearch"（例: "openresearch-dashboard" や "openresearch-mobile"）を含める場合は、そのプロジェクトが OpenResearch チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/openresearch) | [X.com](https://x.com/openresearch)
