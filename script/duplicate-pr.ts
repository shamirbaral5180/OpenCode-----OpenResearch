...
import { createOpenResearch } from "@openresearch-ai/sdk"
...
`)
  ...
  const openresearch = await createOpenResearch({ port: 0 })
  ...
  try {
    ...
    const session = await openresearch.client.session.create()
    const result = await openresearch.client.session
      .prompt({
