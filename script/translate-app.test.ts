...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".openresearch/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".openresearch/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".openresearch/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "openresearch/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
  ...
  test("resolves variants from verbose model output", () => {
    const output = `openresearch/other
{"variants":{}}
openresearch/gpt-5.5
{"variants":{"high":{"reasoningEffort":"high"},"xhigh":{"reasoningEffort":"xhigh"}}}
openresearch/next
{"variants":{}}
`
    expect(modelVariants(output, "openresearch/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "openresearch/gpt-5.5", ["packages/app/src/i18n/fr.ts"])
