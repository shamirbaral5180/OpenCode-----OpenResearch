...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "openresearch/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".openresearch/glossary/zh-cn.md"
  if (locale === "zht") return ".openresearch/glossary/zh-tw.md"
  return `.openresearch/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("OpenResearch did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("OpenResearch returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("OpenResearch session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel OpenResearch runs for 'all' (default: 4)
      --model <provider/id>  OpenResearch model (default: openresearch/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running OpenResearch
...
`)
  ...
  if (failed.length) console.error(`\nOpenResearch failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.OPENRESEARCH_DISABLE_PROJECT_CONFIG = "1"
  env.OPENRESEARCH_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["openresearch", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.OPENRESEARCH_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["openresearch", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.OPENRESEARCH_CONFIG
  delete env.OPENRESEARCH_CONFIG_DIR
  delete env.OPENRESEARCH_CONFIG_CONTENT
  delete env.OPENRESEARCH_PERMISSION
  delete env.OPENRESEARCH_AUTO_SHARE
  return env
}
