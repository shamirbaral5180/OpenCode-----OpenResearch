...
Both `action` and `resource` use openresearch's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.openresearch` files is intentionally deferred until `.openresearch` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether openresearch permits an operation through its managed execution paths.
