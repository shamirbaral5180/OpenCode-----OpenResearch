...
The openresearch server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@openresearch-ai/llm` tool loop after replacing its
...
- plugins get global "openresearch" instance like in that post i showed
- openresearch instance has stuff like `openresearch.session.prompt()` or
  `openresearch.tool.register({...})`
...
- decide whether to preserve deprecated `@openresearch-ai/llm` orchestration exports
