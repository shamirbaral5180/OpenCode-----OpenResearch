...
- Enforced audit pass: with `OPENRESEARCH_RESEARCH_AUDIT=enforced`, the primary research agent receives one bounded verification turn before finalizing that must resolve DOIs, check URLs, verify quotes, update the ledger, and finalize only through `report_write`. This is the default; set `assisted` for prompt-only guidance without a forced turn, or `off` to disable it.
