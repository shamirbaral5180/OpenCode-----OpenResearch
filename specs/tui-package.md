...
Move the canonical OpenResearch terminal application from
`packages/openresearch/src/cli/cmd/tui` into a self-contained workspace package while
...
packages/openresearch ---\
                      > @openresearch-ai/tui -> @openresearch-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/openresearch`,
`packages/cli`, or `@openresearch-ai/core`.

The SDK is the TUI's OpenResearch boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/openresearch`.
- Do not replace private `packages/openresearch` imports with `@openresearch-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@openresearch-ai/tui`.

## Ownership Boundary

### `@openresearch-ai/tui` Owns
...
- OpenResearch domain data displayed by the TUI
...
```text
packages/openresearch/src/cli/cmd/tui
```

Its private dependency on `packages/openresearch` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/openresearch/src/*`.
...
Status: Completed. The private `@openresearch-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@openresearch-ai/tui`.
...
- The package has no dependency on `openresearch`, `@openresearch-ai/cli`, or
  `@openresearch-ai/core`.
...
primitives now live in `@openresearch-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@openresearch-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads OpenResearch globals or process state;
...
Replace process-global OpenResearch state with resolved TUI inputs.
...
- Keep environment reads needed by legacy command or worker startup in
  `packages/openresearch` adapters.
...
`@openresearch-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/openresearch` initially.
...
canonical in `@openresearch-ai/tui`. Configured references resolve through the new
...
adapters rather than OpenResearch domain implementations.

Make the SDK the only OpenResearch domain boundary used by the TUI.
...
- Domain-facing TUI code imports OpenResearch data and operations only from
  `@openresearch-ai/sdk`.
...
`@openresearch-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/openresearch`.
...
- The package has no imports from `packages/openresearch`, `packages/cli`, or
  `@openresearch-ai/core`.
...
public `@openresearch-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/openresearch`.
- Keep the legacy embedded worker and server startup in `packages/openresearch`.
- Change those adapters to load config, create transport inputs, and call the
  public `@openresearch-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/openresearch`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@openresearch-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@openresearch-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for OpenResearch domain state.
...
cd packages/openresearch && bun typecheck
...
rg '@openresearch-ai/core|packages/openresearch|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/openresearch/src packages/openresearch/test
```
...
- Build the current-platform `packages/openresearch` binary.
