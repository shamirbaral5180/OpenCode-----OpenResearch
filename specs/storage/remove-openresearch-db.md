# Remove `packages/openresearch/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/openresearch/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the openresearch legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/openresearch/src/storage/db.ts` are concentrated in 22 source files:

- `packages/openresearch/src/account/repo.ts`
- `packages/openresearch/src/cli/cmd/db.ts`
- `packages/openresearch/src/cli/cmd/import.ts`
- `packages/openresearch/src/cli/cmd/stats.ts`
- `packages/openresearch/src/control-plane/workspace.ts`
- `packages/openresearch/src/index.ts`
- `packages/openresearch/src/node.ts`
- `packages/openresearch/src/permission/index.ts`
- `packages/openresearch/src/project/project.ts`
- `packages/openresearch/src/server/projectors.ts`
- `packages/openresearch/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/openresearch/src/server/shared/fence.ts`
- `packages/openresearch/src/session/message-v2.ts`
- `packages/openresearch/src/session/projectors.ts`
- `packages/openresearch/src/session/prompt.ts`
- `packages/openresearch/src/session/session.ts`
- `packages/openresearch/src/session/todo.ts`
- `packages/openresearch/src/share/share-next.ts`
- `packages/openresearch/src/storage/db.ts`
- `packages/openresearch/src/sync/index.ts`
- `packages/openresearch/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy openresearch database wrapper; `packages/openresearch/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the openresearch wrapper.
...
Status: Completed. `SyncEvent` and the openresearch projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy openresearch database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy openresearch database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy openresearch database wrapper.
...
All migration groups are complete or superseded. `packages/openresearch/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No openresearch data-migration group remains.

The previous openresearch `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate openresearch data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/openresearch`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/openresearch/src`
- `bun typecheck` from `packages/openresearch`
- Relevant package tests from `packages/openresearch`, not the repo root
