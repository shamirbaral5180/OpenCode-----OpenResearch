...
import { createOpenResearchClient } from "@openresearch-ai/sdk/v2/client"
import { ServerAuth } from "@openresearch-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createOpenResearchClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@openresearch/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createOpenResearchClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createOpenResearchClient({ baseUrl: connection.url, headers: connection.headers })
    })
