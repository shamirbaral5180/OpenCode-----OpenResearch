  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://openresearch.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://openresearch.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://openresearch.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://openresearch.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://openresearch.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://openresearch.example.com"),
      ),
    ).toEqual({
