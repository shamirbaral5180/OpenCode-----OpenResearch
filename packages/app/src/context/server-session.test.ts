...
import type { retry } from "@openresearch-ai/core/util/retry"
import type { OpenResearchEvent, SessionApi } from "@openresearch-ai/client/promise"
import type { Message, OpenResearchClient, Part, Session } from "@openresearch-ai/sdk/v2/client"
...
function messageClient(...responses: Array<MessageResponse | Promise<MessageResponse>>) {
  ...
  } as unknown as OpenResearchClient
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as OpenResearchEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as OpenResearchClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
