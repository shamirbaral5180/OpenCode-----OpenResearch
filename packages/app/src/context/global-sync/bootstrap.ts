...
import type {
  ...
  ReferenceListOutput,
  SessionApi,
} from "@openresearch-ai/client/promise"
...
import { NormalizedProviderListResponse } from "@openresearch-ai/session-ui/context"
...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: OpenResearchClient, protocol?: Promise<ServerProtocol>) =>
  queryOptions({
    ...
    queryFn: () =>
      retry(() =>
        api.list().then((projects) => {
          return projects
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("openresearch-test"))
