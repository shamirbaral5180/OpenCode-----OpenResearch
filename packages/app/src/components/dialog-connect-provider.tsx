import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@openresearch-ai/client/promise"
import { Button } from "@openresearch-ai/ui/button"
import { useDialog } from "@openresearch-ai/ui/context/dialog"
import { Dialog } from "@openresearch-ai/ui/dialog"
import { Icon } from "@openresearch-ai/ui/icon"
import { IconButton } from "@openresearch-ai/ui/icon-button"
import { List, type ListRef } from "@openresearch-ai/ui/list"
import { ProviderIcon } from "@openresearch-ai/ui/provider-icon"
import { Spinner } from "@openresearch-ai/ui/spinner"
import { Tag } from "@openresearch-ai/ui/tag"
import { TextField } from "@openresearch-ai/ui/text-field"
import { ButtonV2 } from "@openresearch-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@openresearch-ai/ui/v2/dialog-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "openresearch-go") return language.t("dialog.provider.openresearchGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "openresearch"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.openresearch.tagline")}</div>
          </Show>
          ...
          <Show when={i.id === "openresearch-go"}>
            <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
          </Show>
        </div>
      )}
...
function ProviderPickerV2(props: {
  ...
  const featured = ["openresearch", "openresearch-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "openresearch" || provider.id === "openresearch-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "openresearch"
                                ? "dialog.provider.openresearch.tagline"
                                : "dialog.provider.openresearchGo.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "openresearch" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "openresearch-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "openresearch"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.openresearchZen.line1")}</div>
              <div>{language.t("provider.connect.openresearchZen.line2")}</div>
              <div>
                {language.t("provider.connect.openresearchZen.visit.prefix")}
                <ExternalLink
                  href="https://openresearch.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.openresearchZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.openresearchZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "openresearch"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.openresearchZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.openresearchZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.openresearchZen.visit.prefix")}
                <ExternalLink href="https://openresearch.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.openresearchZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.openresearchZen.visit.suffix")}
              </div>
