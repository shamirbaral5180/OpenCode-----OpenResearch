...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-openresearch" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslOpenResearchAction(check?: WslOpenResearchCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installOpenResearch"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateOpenResearch"
}
...
export function addServerViewModel(input: {
  ...
  const openresearchCheck = selectedDistro ? (state?.openresearchChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.openresearchChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateOpenResearch" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.openresearchMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installOpenResearch" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  openresearchCheck: WslOpenResearchCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingOpenResearch =
    probingSelected ||
    (ready &&
      (!input.openresearchCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingOpenResearch =
    input.state?.job?.kind === "install-openresearch" && input.state.job.distro === input.selectedDistro
  if (!ready || probingOpenResearch) {
    return {
      variant: "contrast",
  ...
  if (!addServerOpenResearchReady(input.openresearchCheck)) {
    const update = !!input.openresearchCheck?.resolvedPath && input.openresearchCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingOpenResearch
        ? { key: "wsl.onboarding.updatingOpenResearch" }
        : update
          ? { key: "wsl.onboarding.updateOpenResearch" }
          : { key: "wsl.onboarding.installOpenResearch" },
      disabled: !!input.state?.job || input.adding,
      action: "install-openresearch",
      loading: installingOpenResearch,
...
function addServerOpenResearchReady(check: WslOpenResearchCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.openresearchChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.openresearchChecks[item.name]) return [`openresearch:${item.name}`]
    return []
  })
