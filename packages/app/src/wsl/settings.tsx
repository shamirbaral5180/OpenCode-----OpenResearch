...
import { wslOpenResearchAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.openresearchChecks[item.config.distro]
          const openresearchAction = () => wslOpenResearchAction(check())
          const busy = () => wsl.data?.job?.kind === "install-openresearch" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={openresearchAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installOpenResearch(item.config.distro))}
                    >
