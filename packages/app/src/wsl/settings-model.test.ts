...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when OpenResearch needs attention", () => {
    expect(wslOpenResearchAction(undefined)).toBeUndefined()
    expect(
      wslOpenResearchAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installOpenResearch")
    expect(
      wslOpenResearchAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/openresearch",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updateOpenResearch")
    expect(
      wslOpenResearchAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/openresearch",
  ...
  test("does not report ready when OpenResearch is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        openresearchChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.openresearch/bin/openresearch",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "openresearch is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installOpenResearch" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-openresearch")
  })
