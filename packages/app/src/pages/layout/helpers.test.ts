...
import { type Session } from "@openresearch-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("openresearch://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("openresearch://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("openresearch://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("openresearch://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("openresearch://open-project")).toBeUndefined()
    expect(parseDeepLink("openresearch://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "openresearch://open-project?directory=/a",
      "openresearch://other?directory=/b",
      "openresearch://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("openresearch://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("openresearch://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("openresearch://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("openresearch://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "openresearch://new-session?directory=/a",
      "openresearch://open-project?directory=/b",
      "openresearch://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENRESEARCH__: {
        deepLinks: ["openresearch://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENRESEARCH__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["openresearch://open-project?directory=/a"])
