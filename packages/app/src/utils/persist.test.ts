...
class MemoryStorage implements Storage {
  ...
  getItem(key: string) {
    ...
    if (key.startsWith("openresearch.throw")) throw new Error("storage get failed")
  ...
  setItem(key: string, value: string) {
    ...
    if (key.startsWith("openresearch.quota")) throw new DOMException("quota", "QuotaExceededError")
    if (key.startsWith("openresearch.throw")) throw new Error("storage set failed")
  ...
  removeItem(key: string) {
    ...
    if (key.startsWith("openresearch.throw")) throw new Error("storage remove failed")
...
describe("persist localStorage resilience", () => {
  test("does not cache values as persisted when quota write and eviction fail", () => {
    const storageApi = persistTesting.localStorageWithPrefix("openresearch.quota.scope")
    storageApi.setItem("value", '{"value":1}')

    expect(storage.getItem("openresearch.quota.scope:value")).toBeNull()
    expect(storageApi.getItem("value")).toBeNull()
  })

  test("disables only the failing scope when storage throws", () => {
    const bad = persistTesting.localStorageWithPrefix("openresearch.throw.scope")
    ...
    const healthy = persistTesting.localStorageWithPrefix("openresearch.safe.scope")
    healthy.setItem("value", '{"value":3}')
    expect(storage.getItem("openresearch.safe.scope:value")).toBe('{"value":3}')
  })

  test("failing fallback scope does not poison direct storage scope", () => {
    const broken = persistTesting.localStorageWithPrefix("openresearch.throw.scope2")
  ...
  test("workspace storage sanitizes Windows filename characters", () => {
    ...
    expect(result).toStartWith("openresearch.workspace.")
  ...
  test("server global target preserves local key and isolates remote keys", () => {
    ...
    expect(Persist.serverGlobal("https://debian.example" as ServerScope, "notification")).toEqual({
      storage: "openresearch.global.dat",
