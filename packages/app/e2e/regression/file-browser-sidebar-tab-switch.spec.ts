...
const directory = "C:/OpenResearch/FileBrowserSidebar"
...
async function setup(
  ...
  await mockOpenResearchServer(page, {
    ...
    provider: {
      ...
      default: { providerID: "openresearch", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "openresearch.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "openresearch.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "openresearch.window.browser.dat:tabs",
