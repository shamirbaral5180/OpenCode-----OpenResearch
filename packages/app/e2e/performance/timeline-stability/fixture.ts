...
import { SessionStatusEvent } from "@openresearch-ai/schema/session-status-event"
import { SessionV1 } from "@openresearch-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockOpenResearchServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/OpenResearch/TimelineStability"
...
export const model = { providerID: "openresearch", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  await mockOpenResearchServer(page, {
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("openresearch.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["openresearch"],
    default: { providerID: "openresearch", modelID: "claude-opus-4-6" },
  }
}
