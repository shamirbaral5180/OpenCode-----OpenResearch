...
import { PromptInput } from "@openresearch-ai/schema/prompt-input"
import { Session } from "@openresearch-ai/schema/session"
import { Project } from "@openresearch-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@openresearch-ai/schema/schema"
...
import { Revert } from "@openresearch-ai/schema/revert"
import { SessionEvent } from "@openresearch-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this OpenResearch process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this OpenResearch process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
