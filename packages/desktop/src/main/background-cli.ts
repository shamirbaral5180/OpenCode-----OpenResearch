...
export async function startBackgroundCli(logger: Logger, _shellStateHome?: string) {
  const stateHome = app.getPath("userData")
  ...
  // Never discover or adopt an upstream service from shell or upstream desktop state.
  const candidates = [stateHome]
...
async function run(
  ...
  env.OPENRESEARCH_HOME = options.stateHome ?? app.getPath("userData")
...
function executableName() {
  return process.platform === "win32" ? "open-research-cli.exe" : "open-research-cli"
}
