...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `openresearch-debug-${stamp()}.zip`)
  try {
...
function serverLogRoots() {
  ...
  return [...new Set([join(xdgData, "openresearch", "log"), join(app.getPath("userData"), "openresearch", "log")])]
}
