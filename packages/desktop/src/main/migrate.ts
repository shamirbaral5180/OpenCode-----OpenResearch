...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "com.srbcorporation.openresearch.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `openresearch.settings.dat` is special: it maps to the `openresearch.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"openresearch.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // openresearch.settings.dat → the electron settings store ("openresearch.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "openresearch.global.dat").
  const storeName = filename === "openresearch.settings.dat" ? "openresearch.settings" : filename
