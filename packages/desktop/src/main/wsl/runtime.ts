...
export async function installWslOpenResearch(_version: string, _distro: string, _opts?: RunWslOptions): Promise<WslCommandResult> {
  throw new Error(nativeT("desktop.fork.wsl.unsupported"))
}
...
export async function resolveWslOpenResearch(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.openresearch/bin/openresearch" ]; then printf "%s\\n" "$HOME/.openresearch/bin/openresearch"; fi',
        distro,
        opts,
      )
    ).stdout,
