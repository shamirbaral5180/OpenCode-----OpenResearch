...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-openresearch", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installOpenResearch(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-openresearch", unavailable)
