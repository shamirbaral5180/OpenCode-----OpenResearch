import type { WslDistroProbe, WslOpenResearchCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  openresearchChecks: Record<string, WslOpenResearchCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextOpenResearchChecks = { ...openresearchChecks }
  delete nextDistroProbes[distro]
  delete nextOpenResearchChecks[distro]
  return { distroProbes: nextDistroProbes, openresearchChecks: nextOpenResearchChecks }
}
