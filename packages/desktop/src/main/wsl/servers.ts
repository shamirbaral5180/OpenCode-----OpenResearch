...
import { expectOpenResearchVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
import { clearWslDistroState, wslServerIdToRestart } from "./policy"
import { nativeT } from "../native-translations"
import {
  installWslDistro,
  installWslOpenResearch,
...
type WslServersControllerOptions = {
  ...
  resolveOpenResearch?: typeof resolveWslOpenResearch
...
export function createWslServersController(
  ...
  const setOpenResearchCheck = (distro: string, check: WslOpenResearchCheck) => {
    setState({
      openresearchChecks: {
        ...state.openresearchChecks,
        [distro]: check,
      },
    })
  }

  const checkOpenResearch = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveOpenResearch ?? resolveWslOpenResearch)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return openresearchCheck(distro, resolved, version, appVersion)
  }

  const refreshOpenResearchCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setOpenResearchCheck(distro, await checkOpenResearch(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const openresearchChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.openresearchChecks[distro])
        .map(async (distro) => [distro, await checkOpenResearch(distro, opts)] as const),
    )
    if (openresearchChecks.length) {
      setState({ openresearchChecks: { ...state.openresearchChecks, ...Object.fromEntries(openresearchChecks) } })
    }
  }
  ...
  const refreshOpenResearchCheckBackground = (id: string, distro: string) => {
    void checkOpenResearch(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setOpenResearchCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl openresearch check failed", { id, distro, message })
      })
  }

  const refreshOpenResearchChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkOpenResearch(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setOpenResearchCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl openresearch check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshOpenResearchCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async initialize() {
      refreshFromStore()
      void refreshOpenResearchChecks()
    ...
    async installOpenResearch(name: string) {
      await runJob({ kind: "install-openresearch", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslOpenResearch(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || nativeT("desktop.wsl.error.installOpenResearch"))
        }
        await refreshOpenResearchCheck(name, { signal: abort.signal })
        expectOpenResearchVersion(state.openresearchChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.openresearchChecks, distro) : {}),
      })
    },
...
function openresearchCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.openresearchMissing"),
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.openresearchCannotRun"),
    }
  }
  return {
    distro,
