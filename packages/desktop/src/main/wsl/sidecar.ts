...
import { type WslCommandLine, resolveWslOpenResearch, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const openresearch = await resolveWslOpenResearch(distro)
  if (!openresearch) throw new Error(nativeT("desktop.wsl.error.openresearchNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "openresearch"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export OPENRESEARCH_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export OPENRESEARCH_CLIENT=desktop",
    `export OPENRESEARCH_SERVER_USERNAME=${shellEscape(username)}`,
    `export OPENRESEARCH_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(openresearch)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
