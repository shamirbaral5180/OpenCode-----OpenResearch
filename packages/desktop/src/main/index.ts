...
const APP_NAMES: Record<string, string> = {
  dev: "OpenResearch Dev",
  beta: "OpenResearch Beta",
  prod: "OpenResearch",
}
...
const TEST_ONBOARDING = process.env.OPENRESEARCH_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.OPENRESEARCH_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : APP_IDS.dev
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `openresearch-onboarding-${randomUUID()}`)
  ...
  const wslServers = createWslServersController(
    app.getVersion(),
    async () => {
      throw new Error("WSL integration is disabled in OpenResearch to prevent OpenResearch service discovery.")
    },
    {
      logger: {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("openresearch://"))
    if (urls.length) {
  ...
  // This fork does not import upstream desktop settings or server connections.
  ...
  app.setAsDefaultProtocolClient("openresearch")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    logger.log("embedded OpenResearch server connection started")

    ensureLoopbackNoProxy()
    useEnvProxy()

    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.OPENRESEARCH_PORT
      if (fromEnv) {
