...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier OpenResearch launch", () => {
    expect(hasExistingAppState([file("openresearch.settings")])).toBe(true)
    expect(hasExistingAppState([file("openresearch.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("openresearch")])).toBe(true)
  })
})
