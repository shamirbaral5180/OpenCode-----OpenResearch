...
function branded(locale: Locale): Dictionary {
  return Object.fromEntries(
    Object.entries(build(locale)).map(([key, value]) => [key, value.replace(/OpenResearch(?:Research)?/g, "OpenResearch")]),
  ) as Dictionary
}

state.dict = branded(state.locale)
...
export function initI18n(): Promise<Locale> {
  ...
  const promise = (async () => {
    const raw = await window.api.storeGet("openresearch.global.dat", "language").catch(() => null)
    const value = parseStored(raw)
    const next = pickLocale(value) ?? state.locale

    state.locale = next
    state.dict = branded(next)
    return next
  })().catch(() => state.locale)
