...
import "./styles.css"
import { Splash } from "@openresearch-ai/ui/logo"
import { useTheme } from "@openresearch-ai/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.OPENRESEARCH_CHANNEL === "prod" &&
...
const deepLinkEvent = "openresearch:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__OPENRESEARCH__ ??= {}
  const pending = window.__OPENRESEARCH__.deepLinks ?? []
  window.__OPENRESEARCH__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `openresearch.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://openresearch.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("openresearch.global.dat").getItem("language")
