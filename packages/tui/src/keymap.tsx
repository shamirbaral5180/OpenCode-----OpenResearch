...
export const OPENRESEARCH_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OPENRESEARCH_MODE_KEY = "openresearch.mode"

export const OpenResearchKeymapProvider = KeymapProvider
export const useOpenResearchKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type OpenResearchModeStack = ReturnType<typeof createOpenResearchModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, OpenResearchModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOpenResearchModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OPENRESEARCH_MODE_KEY, OPENRESEARCH_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OPENRESEARCH_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OPENRESEARCH_MODE_KEY, stack.at(-1)?.mode ?? OPENRESEARCH_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OPENRESEARCH_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OPENRESEARCH_MODE_KEY, undefined)
...
export function useOpenResearchModeStack() {
  return getOpenResearchModeStack(useOpenResearchKeymap())
}

export function getOpenResearchModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("OpenResearch mode stack is not registered for this keymap")
  return value
}
...
export function registerOpenResearchKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createOpenResearchModeStack(keymap)
