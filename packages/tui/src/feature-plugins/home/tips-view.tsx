import type { TuiPluginApi } from "@openresearch-ai/plugin/tui"
...
const NO_MODELS_TIP = "Run {highlight}/connect{/highlight} to add an AI provider and start researching"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("openresearch.status"),
...
const TIPS: Tip[] = [
  ...
  "Include a timeframe and scope in your research question for more relevant evidence",
  (shortcuts) => press(shortcuts.agentCycle(), "to cycle between available agents"),
  "Use {highlight}/undo{/highlight} to revert the last message and file changes",
  "Use {highlight}/redo{/highlight} to restore previously undone messages and file changes",
  "Run {highlight}/share{/highlight} to create a public openresearch.ai link",
  "Drag and drop images or PDFs into the terminal as context",
  (shortcuts) => press(shortcuts.inputPaste(), "to paste images from your clipboard into the prompt"),
  (shortcuts) => `Use ${commandText("/editor", shortcuts.editorOpen())} to compose messages in your external editor`,
  "Ask for primary sources, citations, and a clear distinction between evidence and inference",
  ...
  "Ask for conflicting findings and limitations, not just supporting evidence",
  ...
  "Create {highlight}openresearch.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.openresearch/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Attach source documents and ask for a comparison of their methods and findings",
  "Add {highlight}.md{/highlight} files to {highlight}.openresearch/agents/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "Ask for a source's publication date and whether its findings are still current",
  "Ask for an evidence table with source links and confidence levels",
  "Request a literature review that identifies open questions and research gaps",
  "Ask which claims could not be verified from the available sources",
  "Create {highlight}.ts{/highlight} files in {highlight}.openresearch/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.openresearch/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent OpenResearch from reading sensitive files",
  "Use {highlight}openresearch run{/highlight} for non-interactive scripting",
  "Use {highlight}openresearch --continue{/highlight} to resume the last session",
  "Use {highlight}openresearch run -f notes.md{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}openresearch serve{/highlight} for headless API access to OpenResearch",
  "Use {highlight}openresearch run --attach{/highlight} to connect to a running server",
  "Run {highlight}openresearch upgrade{/highlight} to update to the latest version",
  "Run {highlight}openresearch auth list{/highlight} to see all configured providers",
  "Run {highlight}openresearch agent create{/highlight} for guided agent creation",
  "Break broad research topics into focused subquestions",
  "Ask for exact quotations and page numbers when reviewing a paper",
  "Ask whether multiple sources rely on the same underlying evidence",
  "Request a research report under {highlight}reports/{/highlight} when you need a saved artifact",
  ...
  "Run {highlight}openresearch debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/openresearch{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with OpenResearch Zen for curated, tested models",
  "Use {highlight}AGENTS.md{/highlight} to document research scope and source standards",
  "Ask for a final citation audit before using a research report",
