import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@openresearch-ai/plugin/tui"
...
import type { useOpenResearchKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useOpenResearchKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getOpenResearchModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOpenResearchModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
