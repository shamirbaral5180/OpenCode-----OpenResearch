...
import { getOpenResearchModeStack, OPENRESEARCH_BASE_MODE, OpenResearchKeymapProvider, registerOpenResearchKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerOpenResearchKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when openresearch mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getOpenResearchModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getOpenResearchModeStack(keymap).push("autocomplete")
