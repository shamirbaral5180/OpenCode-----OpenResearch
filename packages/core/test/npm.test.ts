...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@openresearch/acme")).toBe("@openresearch/acme")
    expect(Npm.sanitize("@openresearch/acme@1.0.0")).toBe("@openresearch/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/openresearch/acme.git"
    const expected = win ? "acme@git+https_//github.com/openresearch/acme.git" : spec
