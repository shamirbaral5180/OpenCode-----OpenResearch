...
import { define } from "@openresearch-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@openresearch-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "openresearch.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "openresearch.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
