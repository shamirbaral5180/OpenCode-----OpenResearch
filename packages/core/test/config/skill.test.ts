...
import { ConfigSkillPlugin } from "@openresearch-ai/core/config/plugin/skill"
    ...
    Effect.gen(function* () {
      ...
      yield* ConfigSkillPlugin.Plugin.effect(
        ...
        Effect.provideService(
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Directory({ type: "directory", path: AbsolutePath.make("/repo/.openresearch") }),
                new Config.Document({
                  type: "document",
                  info: decode({
      ...
      expect(sources).toEqual([
        SkillV2.DirectorySource.make({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.openresearch", "skill")),
        }),
        SkillV2.DirectorySource.make({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.openresearch", "skills")),
        }),
