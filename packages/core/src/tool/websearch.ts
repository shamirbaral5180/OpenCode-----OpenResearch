...
import { ToolFailure } from "@openresearch-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@openresearch/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.OPENRESEARCH_WEBSEARCH_PROVIDER === "exa" || process.env.OPENRESEARCH_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.OPENRESEARCH_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("OPENRESEARCH_EXPERIMENTAL") || truthy("OPENRESEARCH_ENABLE_EXA") || truthy("OPENRESEARCH_EXPERIMENTAL_EXA"),
    enableParallel: truthy("OPENRESEARCH_ENABLE_PARALLEL") || truthy("OPENRESEARCH_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `openresearch/${InstallationVersion}`,
