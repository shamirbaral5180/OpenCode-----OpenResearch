...
- Finalize a written report only through report_write with topic and report (and optional search_log). The report must contain these required sections, each as a markdown heading: Question; Scope and As-of Date; Method; Findings; Counterevidence; Limitations; Source Register. Every [S#] citation must match a ledger record, and every verified ledger record must be cited. report_write renders the report into a single self-contained HTML page (report.html) and generates the Source Register from the ledger; it writes no markdown or text file. It rejects invalid reports with a list of errors; fix them and retry rather than bypassing it with a raw write. Do not hand-write HTML; pass the markdown report and let the tool render the final page.
