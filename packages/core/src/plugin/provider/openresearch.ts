...
import type { IntegrationOAuthMethodRegistration } from "@openresearch-ai/plugin/v2/effect/integration"
import { define } from "@openresearch-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@openresearch-ai/sdk/v2/types"
...
const defaultServer = "https://openresearch.ai/console"
const clientID = "openresearch-cli"
...
export const OpenResearchPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "openresearch",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpenResearchPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("openresearch")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load OpenResearch provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("openresearch", (integration) => {
        integration.name = "OpenResearch"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "openresearch", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("openresearch")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("openresearch")
      ...
      const item = catalog.provider.get(ProviderV2.ID.openresearch)
      if (!item) return
      const hasKey = Boolean(process.env.OPENRESEARCH_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("openresearch")),
