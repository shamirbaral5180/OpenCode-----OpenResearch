...
import { isSessionNotFoundError, isUnauthorizedError, OpenResearch } from "../src"

test("exposes every standard HTTP API group", () => {
  const client = OpenResearch.make({ baseUrl: "http://localhost:3000" })
...
test("sessions.get returns the wire projection", async () => {
  const client = OpenResearch.make({
