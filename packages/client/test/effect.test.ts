...
import { AbsolutePath, Agent, Location, Model, OpenResearch, Prompt, Session, SessionMessage } from "../src/effect"
...
test("sessions.get returns the decoded Effect projection", async () => {
  ...
  const result = await Effect.gen(function* () {
    const client = yield* OpenResearch.make({ baseUrl: "http://localhost:3000" })
