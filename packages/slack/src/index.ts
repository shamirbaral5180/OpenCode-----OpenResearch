...
import { createOpenResearch, type ToolPart } from "@openresearch-ai/sdk"
...
console.log("🚀 Starting openresearch server...")
const openresearch = await createOpenResearch({
  port: 0,
})
console.log("✅ OpenResearch server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await openresearch.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new openresearch session...")
    const { client, server } = openresearch
    ...
    console.log("✅ Created openresearch session:", createResult.data.id)
  ...
  console.log("📝 Sending to openresearch:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 OpenResearch response:", JSON.stringify(result, null, 2))
