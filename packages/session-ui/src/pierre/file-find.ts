...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("openresearch-find")
  api.delete("openresearch-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("openresearch-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("openresearch-find", new Highlight(...rest))
    return true
  }
