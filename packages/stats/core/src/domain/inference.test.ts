...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve openresearch route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "openresearch")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "openresearch")).toBe("openai")
    expect(statProvider("big-pickle", "", "openresearch")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "openresearch"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep openresearch as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "openresearch"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "openresearch"))).toMatchObject([{ provider: "unknown" }])
    expect(toProviderAggregate(aggregate("muse-spark-1.2-contributor", "unknown"))).toMatchObject([
      { provider: "meta" },
    ])
  })

  test("geo aggregates never keep openresearch or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "openresearch"), country: "US" })).toMatchObject([
