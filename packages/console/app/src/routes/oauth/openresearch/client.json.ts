...
// OAuth Client ID Metadata Document for the openresearch client.
// Spec: https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/
//
// When an MCP server's authorization server supports this, openresearch sends this URL as its OAuth client_id
...
// built from the request origin and stays valid on dev.openresearch.ai as well as production.
//
// redirect_uris have no port because openresearch binds an ephemeral port per login. RFC 8252 section 7.3 has
// authorization servers ignore the port when matching loopback redirects for native apps.
const PATH = "/oauth/openresearch/client.json"
