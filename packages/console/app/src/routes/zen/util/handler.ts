...
import { and, Database, eq, isNull, lt, or, sql } from "@openresearch-ai/console-core/drizzle/index.js"
import { KeyTable } from "@openresearch-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@openresearch-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@openresearch-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@openresearch-ai/console-core/util/date.js"
import { Identifier } from "@openresearch-ai/console-core/identifier.js"
import { Billing } from "@openresearch-ai/console-core/billing.js"
import { Actor } from "@openresearch-ai/console-core/actor.js"
import { WorkspaceTable } from "@openresearch-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@openresearch-ai/console-core/model.js"
import { Subscription } from "@openresearch-ai/console-core/subscription.js"
import { BlackData } from "@openresearch-ai/console-core/black.js"
import { UserTable } from "@openresearch-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@openresearch-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@openresearch-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@openresearch-ai/console-core/lite.js"
import { Resource } from "@openresearch-ai/console-resource"
...
import { Workspace } from "@openresearch-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-openresearch-session") ?? ""
    const requestId = input.request.headers.get("x-openresearch-request") ?? ""
    const ocClient = input.request.headers.get("x-openresearch-client") ?? ""
    const projectId = input.request.headers.get("x-openresearch-project") ?? ""
    ...
    if (authInfo && opts.modelList === "lite" && requiresGoTrainingConsent(modelInfo.id) && !authInfo.allowTraining)
      throw new DataPolicyError(
        t("zen.api.error.trainingNotAllowed", {
          consoleGoUrl: `https://openresearch.ai/workspace/${authInfo.workspaceID}/go`,
        }),
      )
    ...
    const providerRequest = async () => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          if (!isNewInference) {
            headers.delete("x-openresearch-session")
            headers.delete("x-openresearch-project")
            headers.delete("x-openresearch-client")
            headers.delete("x-openresearch-request")
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-openresearch-endpoint-id")
        const resEndpointModelId = res.headers.get("x-openresearch-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      ...
      try {
        const consoleGoUrl = `https://openresearch.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://openresearch.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://openresearch.ai/workspace/${authInfo.workspaceID}/members`
