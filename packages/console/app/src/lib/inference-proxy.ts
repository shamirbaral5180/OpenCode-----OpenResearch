...
import { and, Database, eq, isNull, sql } from "@openresearch-ai/console-core/drizzle/index.js"
...
export async function proxyInference(
  ...
  const requestID = request.headers.get("x-openresearch-request-id") ?? request.headers.get("x-openresearch-request")
  if (requestID) forwarded.headers.set("x-openresearch-request-id", requestID)
