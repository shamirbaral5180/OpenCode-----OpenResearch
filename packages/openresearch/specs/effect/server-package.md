...
Practical reference for a future `packages/server` split after the openresearch
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/openresearch`.
...
Until enough shared service code lives outside `packages/openresearch`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/openresearch`

It should not import `packages/openresearch` services while `packages/openresearch`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on openresearch-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/openresearch` implementation details.
