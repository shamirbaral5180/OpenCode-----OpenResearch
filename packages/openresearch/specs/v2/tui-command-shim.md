...
Delete `packages/openresearch/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/openresearch/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/openresearch/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/openresearch`
- TUI plugin loader tests in `packages/openresearch` if runtime plugin API wiring changed
