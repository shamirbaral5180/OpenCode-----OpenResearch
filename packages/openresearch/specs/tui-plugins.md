...
- Author package entrypoint is `@openresearch-ai/plugin/tui`.
...
```json
{
  "$schema": "https://openresearch.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/openresearch-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
    ...
    "sound": true,
    "volume": 0.4,
    "sound_pack": "openresearch.default",
    "sounds": {
...
- Import types from `@openresearch-ai/plugin/tui`.
- `@openresearch-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@openresearch-ai/plugin/tui"
...
- The value is a semver range checked against the running OpenResearch version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.openresearch` is absent, no check is performed (backward compatible).
...
- `openresearch plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `openresearch plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.openresearch` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.openresearch`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.openresearch`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.openresearch` dir or `OPENRESEARCH_CONFIG_DIR`, OpenResearch installs `@openresearch-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@openresearch-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus OpenResearch's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
OpenResearch registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, OpenResearch pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@openresearch-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"openresearch"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `openresearch.default`.
...
- Local plugins persist installed themes under the local `.openresearch/themes` area near the plugin config source.
...
- `--pure` / `OPENRESEARCH_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.openresearch/plugins/tui-smoke.tsx`
- Local vim plugin: `.openresearch/plugins/tui-vim.tsx`
- Local smoke config: `.openresearch/tui.json`
- Local smoke theme: `.openresearch/plugins/smoke-theme.json`
