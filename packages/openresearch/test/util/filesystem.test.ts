...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "openresearch.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "openresearch.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openresearch.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openresearch.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["openresearch.json", "openresearch.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "openresearch.json"),
        path.join(project, "openresearch.jsonc"),
      ])
    })
  })
