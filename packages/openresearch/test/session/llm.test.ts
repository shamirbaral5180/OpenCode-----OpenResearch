...
import { LLMClient, RequestExecutor } from "@openresearch-ai/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const openresearchFixture = { providerID: "openresearch-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for openresearch providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(openresearchFixture.providerID),
          ModelV2.ID.make(openresearchFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(openresearchFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [openresearchFixture.providerID],
          provider: {
            [openresearchFixture.providerID]: {
              name: "OpenResearch Test",
