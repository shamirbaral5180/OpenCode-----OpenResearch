...
import { ModelsDev } from "@openresearch-ai/core/models-dev"
import { HttpRecorder } from "@openresearch-ai/http-recorder"
import { HttpRecorderInternal } from "@openresearch-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@openresearch-ai/llm"
import { RequestExecutor } from "@openresearch-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.openresearch.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.OPENRESEARCH_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["openresearch", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OPENRESEARCH_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("OPENRESEARCH_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["openresearch", "native", "oauth", "tool-loop"],
  ...
  {
    id: "openresearch-proxy",
    name: "OpenResearch proxy",
    providerID: ProviderV2.ID.openresearch,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["openresearch", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.OPENRESEARCH_RECORD_CONSOLE_TOKEN && process.env.OPENRESEARCH_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openresearch,
        name: "OpenResearch Zen",
        env: ["OPENRESEARCH_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.OPENRESEARCH_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.OPENRESEARCH_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.OPENRESEARCH_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["openresearch", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OPENRESEARCH_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("OPENRESEARCH_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("OPENRESEARCH_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set OPENRESEARCH_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the openresearch LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "openresearch.json"),
      JSON.stringify({ $schema: "https://openresearch.ai/config.json", ...scenario.config(model) }),
    ),
  )
