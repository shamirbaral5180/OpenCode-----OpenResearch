...
import { SessionV1 } from "@openresearch-ai/core/v1/session"
...
import { SessionMessageTable } from "@openresearch-ai/core/session/sql"
...
import { SessionV2 } from "@openresearch-ai/core/session"
import { SessionExecution } from "@openresearch-ai/core/session/execution"
import { Skill } from "../../src/skill"
import { SystemPrompt } from "../../src/session/system"
import { Shell } from "@openresearch-ai/core/shell"
import { Snapshot } from "../../src/snapshot"
import { ToolRegistry } from "@/tool/registry"
import { Truncate } from "@/tool/truncate"
import { CrossSpawnSpawner } from "@openresearch-ai/core/cross-spawn-spawner"
import { Ripgrep } from "@openresearch-ai/core/ripgrep"
import { Format } from "../../src/format"
import { TestInstance } from "../fixture/fixture"
import { awaitWithTimeout, pollWithTimeout, testEffect } from "../lib/effect"
import { reply, TestLLMServer } from "../lib/llm-server"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { ProviderV2 } from "@openresearch-ai/core/provider"
import { ModelV2 } from "@openresearch-ai/core/model"
...
const runtimeFlags = RuntimeFlags.layer({ experimentalEventSystem: true, researchAudit: "off" })
...
const withMcpInstructions = testEffect(
  makeHttp({
    mcpInstructions: [
      {
        name: "guide-server",
        // Advertise a reviewed retrieval operation so the locked research policy
        // permits the server's instructions to surface in context.
        instructions: "Use lookup before mutate.",
        tools: ["guide-server_web_search_exa"],
      },
    ],
  }),
)
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://openresearch.ai/config.json", ...config }),
  )
})
...
// The locked research policy removes the legacy coding agents (build/plan). Hidden
// internal agents still resolve and can resume a persisted session.
for (const agent of ["title", "summary"]) {
  it.instance(`loop resumes persisted messages using hidden internal ${agent}`, () =>
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const agents = yield* AgentSvc.Service
      const chat = yield* sessions.create({ title: "Hidden", agent })
      expect((yield* agents.get(agent)).hidden).toBe(true)
      yield* prompt.prompt({
        sessionID: chat.id,
        agent,
        noReply: true,
      ...
      const result = yield* prompt.loop({ sessionID: chat.id })
      expect(result.info.agent).toBe(agent)
      expect((yield* sessions.get(chat.id)).agent).toBe(agent)
    }),
  )
}

it.instance("loop rejects a removed legacy coding agent", () =>
  Effect.gen(function* () {
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const chat = yield* sessions.create({ title: "Legacy" })
    ...
    if (Exit.isSuccess(exit)) throw new Error("expected failure for removed agent")
    const message = JSON.stringify(Cause.squash(exit.cause))
    expect(message).toContain("Agent not found")
  }),
)

it.instance("default research prompt replaces the provider coding prompt in model requests", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const chat = yield* sessions.create({ title: "Research" })
    yield* prompt.prompt({
      sessionID: chat.id,
      noReply: true,
      parts: [{ type: "text", text: "Investigate this question" }],
    })
    yield* llm.text("Research result")
    const result = yield* prompt.loop({ sessionID: chat.id })
    expect(result.info.agent).toBe("research")
    const body = JSON.stringify((yield* llm.hits)[0]?.body)
    expect(body).toContain("You are OpenResearch")
    expect(body).toContain("Evidence ledger:")
    const provider = yield* ProviderSvc.Service
    const model = yield* provider.getModel(ref.providerID, ref.modelID)
    for (const coding of SystemPrompt.provider(model)) {
      expect(body).not.toContain(JSON.stringify(coding).slice(1, -1))
    }
  }),
)
...
it.instance("completed subtask records session and model metadata on the tool part", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    ...
    const taskMsg = msgs.find((item) => item.info.role === "assistant" && item.info.agent === "research-scout")
...
it.instance("subtask child inherits parent session external_directory allow", () =>
  Effect.gen(function* () {
    ...
    // Research workers cannot delegate; unknown subagents are denied by their own ruleset.
    const worker = yield* (yield* AgentSvc.Service).get("research-scout")
    const combined = Permission.merge(worker!.permission, rules)
    expect(Permission.evaluate("task", "anything", combined).action).toBe("deny")
  }),
)
  ...
  () =>
    Effect.gen(function* () {
      ...
      const tool = yield* pollWithTimeout(
        Effect.gen(function* () {
          const msgs = yield* MessageV2.filterCompactedEffect(chat.id)
          const assistant = msgs.findLast((item) => item.info.role === "assistant" && item.info.agent === "research")
...
it.instance("shell rejects with BusyError when loop running", () =>
  Effect.gen(function* () {
    ...
    const exit = yield* prompt.shell({ sessionID: chat.id, agent: "research", command: "echo hi" }).pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
    if (Exit.isFailure(exit)) {
      expect(Cause.squash(exit.cause)).toBeInstanceOf(Session.BusyError)
      expect(Cause.squash(exit.cause)).toMatchObject({ _tag: "SessionBusyError", sessionID: chat.id })
    ...
    withSh(() =>
      Effect.gen(function* () {
        ...
        const fiber = yield* prompt
          .shell({ sessionID: chat.id, agent: "research", command: "printf first && sleep 0.2 && printf second" })
          .pipe(Effect.forkChild)
  ...
  () =>
    Effect.gen(function* () {
      ...
      const sh = yield* prompt
        .shell({ sessionID: chat.id, agent: "research", command: "sleep 0.2" })
        .pipe(Effect.forkChild)
      yield* waitForBusy(chat.id)
  ...
  () =>
    withSh(() =>
      Effect.gen(function* () {
        ...
        const sh = yield* prompt
          .shell({ sessionID: chat.id, agent: "research", command: ": > '.shell-ready'; sleep 30" })
          .pipe(Effect.forkChild)
        yield* pollWithTimeout(
...
unixNoLLMServer(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const sh = yield* prompt.shell({ sessionID: chat.id, agent: "research", command: "sleep 30" }).pipe(Effect.forkChild)
      yield* waitForBusy(chat.id)
  ...
  () =>
    withSh(() =>
      Effect.gen(function* () {
        ...
        const a = yield* prompt
          .shell({ sessionID: chat.id, agent: "research", command: "sleep 30" })
          .pipe(Effect.forkChild)
        yield* waitForBusy(chat.id)
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/openresearch/src/session/processor.ts")
...
noLLMServer.instance(
  "research agent ignores user-configured agent model and reuses the session model",
  () =>
    Effect.gen(function* () {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "research",
        model: { providerID: ProviderV2.ID.make("openresearch"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
      ...
      // The locked research agent has no model/variant of its own, so an omitted
      // model reuses the session's current model rather than any user agent config.
      ...
      expect(match.info.model).toEqual({
        providerID: ProviderV2.ID.make("openresearch"),
    ...
    }),
  {
    config: {
      ...
      // The locked policy removes build and refuses per-agent model overrides.
      agent: {
        build: {
  ...
  () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit)) {
        ...
        expect(NamedError.Unknown.isInstance(err)).toBe(true)
        if (NamedError.Unknown.isInstance(err)) {
          expect(err.data.message).toContain("research")
          expect(err.data.message).not.toContain("build")
          expect(err.data.message).not.toContain("plan")
        }
      }
    }),
  30_000,
