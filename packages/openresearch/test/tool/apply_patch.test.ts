...
describe("tool.apply_patch freeform", () => {
  ...
  it.instance(
    "denies moves outside reports before any patch mutations",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const permission = yield* Permission.Service
        const original = path.join(test.directory, "reports/draft.md")
        const destination = path.join(test.directory, "src/example.ts")
        yield* makeDir(path.dirname(original))
        yield* makeDir(path.dirname(destination))
        yield* writeText(original, "draft\n")
        yield* writeText(destination, "export const original = true\n")

        yield* expectFailure(
          execute(
            {
              patchText:
                "*** Begin Patch\n*** Add File: reports/new/created.md\n+created\n*** Update File: reports/draft.md\n*** Move to: src/example.ts\n@@\n-draft\n+replaced\n*** End Patch",
            },
            {
              ...baseCtx,
              ask: (input) =>
                permission
                  .ask({
                    ...input,
                    sessionID: baseCtx.sessionID,
                    ruleset: Permission.fromConfig({ edit: { "*": "deny", "reports/**": "allow" } }),
                  })
                  .pipe(Effect.orDie),
            },
          ),
        ...
        expect(yield* readText(original)).toBe("draft\n")
        expect(yield* readText(destination)).toBe("export const original = true\n")
        yield* expectFailure(Effect.promise(() => fs.stat(path.join(test.directory, "reports/new"))))
      }),
    { git: true },
  )

  it.instance(
    "allows moves within reports with report-only edit permission",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const permission = yield* Permission.Service
        const original = path.join(test.directory, "reports/draft.md")
        const destination = path.join(test.directory, "reports/final/report.md")
        yield* makeDir(path.dirname(original))
        yield* writeText(original, "draft\n")
        const calls: AskInput[] = []
        ...
        yield* execute(
          {
            patchText:
              "*** Begin Patch\n*** Update File: reports/draft.md\n*** Move to: reports/final/report.md\n@@\n-draft\n+final\n*** End Patch",
          },
          {
            ...baseCtx,
            ask: (input) => {
        ...
        expect(calls[0].patterns).toEqual(["reports/draft.md", "reports/final/report.md"])
        expect(calls[0].metadata.filepath).toBe("reports/draft.md, reports/final/report.md")
        yield* expectReadFailure(original)
        expect(yield* readText(destination)).toBe("final\n")
      }),
    { git: true },
  )
