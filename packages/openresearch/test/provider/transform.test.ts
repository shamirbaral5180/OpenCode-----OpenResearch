    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".openresearch/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const openresearchModel = {
      ...openaiModel,
      providerID: "openresearch",
      api: {
        id: "openresearch-test",
        url: "https://api.openresearch.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, openresearchModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.openresearch?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.openresearch?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          openresearch: { itemId: "msg_openresearch" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              openresearch: { itemId: "msg_openresearch_part" },
    ...
    expect(result[0].providerOptions?.openresearch?.itemId).toBe("msg_openresearch")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.openresearch?.itemId).toBe("msg_openresearch_part")
...
describe("ProviderTransform sampling defaults - DeepSeek", () => {
  ...
  test.each([
    ...
    ["openresearch", "deepseek-v4-flash"],
    ["openresearch-go", "deepseek-v4-flash"],
