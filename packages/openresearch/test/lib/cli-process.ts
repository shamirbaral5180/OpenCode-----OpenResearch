// Subprocess test harness for the openresearch CLI. Spawns the real binary against
...
//
// Configuration flows through openresearch's built-in test affordances:
//   - OPENRESEARCH_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENRESEARCH_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENRESEARCH_DISABLE_PROJECT_CONFIG : skip walking up for openresearch.json
//   - OPENRESEARCH_PURE                : skip external plugin discovery + install
//   - OPENRESEARCH_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `openresearch.run` is fully wired. The shape supports adding more
// builders (`openresearch.serve(opts)`, `openresearch.acp(opts)`, `openresearch.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpenResearchCli.
...
import { AppProcess } from "@openresearch-ai/core/process"
...
const openresearchRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(openresearchRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    OPENRESEARCH_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    OPENRESEARCH_CONFIG_CONTENT: configJson,
    OPENRESEARCH_DISABLE_PROJECT_CONFIG: "1",
    OPENRESEARCH_PURE: "1",
    OPENRESEARCH_DISABLE_AUTOUPDATE: "1",
    OPENRESEARCH_DISABLE_AUTOCOMPACT: "1",
    OPENRESEARCH_DISABLE_MODELS_FETCH: "1",
    OPENRESEARCH_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `openresearch run`. New flags should
...
// `openresearch serve` is a long-lived process — it never exits on its own.
...
// `openresearch acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpenResearchCli = {
  ...
  // Spawn `openresearch serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `openresearch acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly openresearch: OpenResearchCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("openresearch.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          OPENRESEARCH_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("openresearch.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("openresearch.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "openresearch server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `openresearch serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("openresearch.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const openresearch: OpenResearchCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, openresearch })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `openresearch.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "openresearch") {
...
// every call site. Use this for any test that needs the openresearch CLI fixture.
//
...
// resources (e.g. `openresearch.serve`) without an extra `Effect.scoped` wrapper —
