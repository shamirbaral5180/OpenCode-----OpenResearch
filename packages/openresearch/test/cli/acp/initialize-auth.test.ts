...
describe("openresearch acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ openresearch }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ openresearch }))
        ...
        expect(initialized.agentInfo?.name).toBe("OpenResearch")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openresearch }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ openresearch })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("openresearch-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "openresearch-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.OPENRESEARCH_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openresearch }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("openresearch-login")
