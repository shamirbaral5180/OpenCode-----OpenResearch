// Subprocess integration tests for `openresearch run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `openresearch.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `OPENRESEARCH_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("openresearch run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, openresearch }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openresearch.run("say hi")
        openresearch.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, openresearch }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openresearch.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, openresearch }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* openresearch.run("think", { extraArgs: ["--thinking"] })
        openresearch.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* openresearch.run("think again")
        openresearch.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, openresearch }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openresearch.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, openresearch }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openresearch.run("say hi", { format: "json" })
        openresearch.expectExit(result, 0)

        const events = openresearch.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ openresearch }) =>
      Effect.gen(function* () {
        const result = yield* openresearch.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, openresearch }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openresearch.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, openresearch }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* openresearch.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, openresearch }) =>
      Effect.gen(function* () {
        yield* llm.tool("bash", { command: "rm -f denied-file", description: "Remove a test file" })
        yield* llm.text("continued after rejection")
        const denied = yield* openresearch.run("request permission", { permission: { bash: "ask" } })
        openresearch.expectExit(denied, 0)
        ...
        const allowed = yield* openresearch.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        openresearch.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* openresearch.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        openresearch.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, openresearch }) =>
      Effect.gen(function* () {
        ...
        const server = yield* openresearch.serve()

        const result = yield* openresearch.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, openresearch }) =>
      Effect.gen(function* () {
        const result = yield* openresearch.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, openresearch }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* openresearch.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
