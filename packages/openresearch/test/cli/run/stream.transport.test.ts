...
import { OpenResearchClient, type GlobalEvent } from "@openresearch-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<OpenResearchClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<OpenResearchClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<OpenResearchClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<OpenResearchClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<OpenResearchClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://openresearch.test"),
...
function sdk(
  input: {
    ...
    subscribe?: OpenResearchClient["event"]["subscribe"]
    globalEvent?: OpenResearchClient["global"]["event"]
    promptAsync?: OpenResearchClient["session"]["promptAsync"]
    status?: OpenResearchClient["session"]["status"]
    messages?: OpenResearchClient["session"]["messages"]
    children?: OpenResearchClient["session"]["children"]
    permissions?: OpenResearchClient["permission"]["list"]
    questions?: OpenResearchClient["question"]["list"]
  } = {},
) {
  const client = new OpenResearchClient()

  const subscribe: OpenResearchClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: OpenResearchClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: OpenResearchClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: OpenResearchClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: OpenResearchClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: OpenResearchClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: OpenResearchClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: OpenResearchClient["question"]["list"] = input.questions ?? (() => ok([]))
