...
describe("openresearch read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // OPENRESEARCH_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ openresearch }) =>
      Effect.gen(function* () {
        const r = yield* openresearch.spawn(["mcp", "list"])
        openresearch.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openresearch }) =>
      Effect.gen(function* () {
        const r = yield* openresearch.spawn(["providers", "list"])
        openresearch.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ openresearch }) =>
      Effect.gen(function* () {
        const r = yield* openresearch.spawn(["models"])
        openresearch.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ openresearch }) =>
      Effect.gen(function* () {
        const r = yield* openresearch.spawn(["agent", "list"])
        openresearch.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh OPENRESEARCH_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ openresearch }) =>
      Effect.gen(function* () {
        const r = yield* openresearch.spawn(["session", "list"])
        openresearch.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openresearch }) =>
      Effect.gen(function* () {
        const r = yield* openresearch.spawn(["stats"])
        openresearch.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under OPENRESEARCH_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ openresearch }) =>
      Effect.gen(function* () {
        const r = yield* openresearch.spawn(["db", "path"])
        openresearch.expectExit(r, 0, "db path")
