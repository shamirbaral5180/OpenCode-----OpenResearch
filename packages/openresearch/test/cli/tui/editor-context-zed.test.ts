...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@openresearch-ai/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.OPENRESEARCH_ZED_DB
  process.env.OPENRESEARCH_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.OPENRESEARCH_ZED_DB
    else process.env.OPENRESEARCH_ZED_DB = previous
    home.mockRestore()
  }
})
