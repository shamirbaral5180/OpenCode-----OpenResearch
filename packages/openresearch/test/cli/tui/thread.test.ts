...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ openresearch }) =>
    Effect.gen(function* () {
      const result = yield* openresearch.spawn(["--replay-limit", "10"])

      openresearch.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ openresearch }) =>
    Effect.gen(function* () {
      const result = yield* openresearch.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ openresearch }) =>
    Effect.gen(function* () {
      const result = yield* openresearch.spawn(["--mini", "--port", "4096"])
