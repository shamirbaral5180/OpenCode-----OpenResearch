...
const globalConfigFiles = ["openresearch.json", "openresearch.jsonc", "tui.json", "tui.jsonc"].map((file) =>
  path.join(Global.Path.config, file),
)

const cleanState = Effect.gen(function* () {
  const fs = yield* FSUtil.Service
  delete process.env.OPENRESEARCH_CONFIG
  delete process.env.OPENRESEARCH_TUI_CONFIG
...
it.instance("keeps server and tui plugin merge semantics aligned", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const local = path.join(test.directory, ".openresearch")
      yield* fs.makeDirectory(local, { recursive: true })

      yield* fs.writeJson(path.join(Global.Path.config, "openresearch.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(local, "openresearch.json"), {
  ...
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(
        path.join(test.directory, ".openresearch", "tui.json"),
...
it.instance("migrates tui-specific keys from openresearch.json when tui.json does not exist", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      const source = path.join(test.directory, "openresearch.json")
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "openresearch.json.tui-migration.bak"))).toBe(true)
...
it.instance("migrates project legacy tui keys even when global tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), { theme: "global" })
      yield* fs.writeJson(path.join(test.directory, "openresearch.json"), {
      ...
      const server = JSON.parse(yield* fs.readFileString(path.join(test.directory, "openresearch.json")))
      expect(server.theme).toBeUndefined()
      expect(server.tui).toBeUndefined()
    }),
  ),
...
it.instance("skips migration when openresearch.jsonc is syntactically invalid", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
  ...
  "username": "still-broken"
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "openresearch.jsonc.tui-migration.bak"))).toBe(false)
      const source = yield* fs.readFileString(path.join(test.directory, "openresearch.jsonc"))
...
it.instance("skips migration when tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(test.directory, "openresearch.json"), { theme: "legacy" })
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "openresearch.json.tui-migration.bak"))).toBe(false)
    }),
  ),
)
...
it.instance("migration backup preserves JSONC comments", () =>
  ...
  "tui": {
    ...
    "scroll_speed": 1.5
      ...
      const backup = yield* fs.readFileString(path.join(test.directory, "openresearch.jsonc.tui-migration.bak"))
...
it.instance("migrates legacy tui keys across multiple openresearch.json levels", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(test.directory, "openresearch.json"), { theme: "root-theme" })
      yield* fs.writeJson(path.join(nested, "openresearch.json"), { theme: "nested-theme" })
...
it.instance("project config takes precedence over OPENRESEARCH_TUI_CONFIG (matches OPENRESEARCH_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("OPENRESEARCH_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from OPENRESEARCH_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(customDir, "openresearch.json"), { model: "test/model" })
      yield* fs.writeJson(path.join(customDir, "tui.json"), { theme: "should-not-load" })

      yield* withEnv(
        "OPENRESEARCH_CONFIG",
        path.join(customDir, "openresearch.json"),
        Effect.gen(function* () {
          const config = yield* getTuiConfig(test.directory)
          expect(config.theme).toBeUndefined()
        }),
...
it.instance("loads .openresearch/tui.json", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("silently skips malformed tui.json - load failures degrade to {}", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(path.join(test.directory, ".openresearch", "tui.json"), JSON.stringify({ theme: "fallback" }))
