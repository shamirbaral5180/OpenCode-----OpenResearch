...
import type { Event, OpenResearchClient } from "@openresearch-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<OpenResearchClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "openresearch-acp-permission-"))
