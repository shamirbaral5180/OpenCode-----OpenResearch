...
import type { HostPluginApi } from "@openresearch-ai/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createOpenResearchClient({
  ...
  let selected = opts.theme?.selected ?? "openresearch"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "openresearch.default",
