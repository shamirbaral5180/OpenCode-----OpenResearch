...
const withoutWatcher = <A, E, R>(effect: Effect.Effect<A, E, R>) => {
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.OPENRESEARCH_EXPERIMENTAL_DISABLE_FILEWATCHER
      process.env.OPENRESEARCH_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
      return previous
    }),
    () => effect,
    (previous) =>
      Effect.sync(() => {
        if (previous === undefined) delete process.env.OPENRESEARCH_EXPERIMENTAL_DISABLE_FILEWATCHER
        else process.env.OPENRESEARCH_EXPERIMENTAL_DISABLE_FILEWATCHER = previous
      }),
  )
}
