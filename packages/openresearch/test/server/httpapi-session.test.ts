...
import { SessionInputTable, SessionMessageTable, SessionTable } from "@openresearch-ai/core/session/sql"
...
const originalWorkspaces = Flag.OPENRESEARCH_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.OPENRESEARCH_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-openresearch-directory": test.directory }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(
          yield* requestJson<null>(pathFor(SessionPaths.modelInput, { sessionID: parent.id }), { headers }),
        ).toBeNull()
    ...
    () =>
      Effect.gen(function* () {
        ...
        const messages = yield* request(`/api/session/${session.id}/message`, {
          headers: { "x-openresearch-directory": test.directory },
        })
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-openresearch-directory": test.directory, "content-type": "application/json" }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.OPENRESEARCH_EXPERIMENTAL_WORKSPACES = true
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-openresearch-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "openresearch", "src")
        ...
        const query = new URLSearchParams({
          scope: "project",
          path: "packages/openresearch/src",
          directory: currentDir,
        })
        const headers = { "x-openresearch-directory": test.directory }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-openresearch-directory": hint, "content-type": "application/json" }
        const created = yield* requestJson<Session.Info>(SessionPaths.create, {
          method: "POST",
          headers,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-openresearch-directory": globalWorktreeSentinel, "content-type": "application/json" }
