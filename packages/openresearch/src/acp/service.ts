...
import type { AssistantMessage, Message, OpenResearchClient, SessionMessageResponse } from "@openresearch-ai/sdk/v2"
...
export const AuthMethodID = "openresearch-login"
...
export class Service extends Context.Service<Service, Interface>()("@openresearch/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `openresearch auth login` in the terminal",
      name: "Login with openresearch",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: OpenResearchClient) {
...
function makeUsageService(sdk: OpenResearchClient) {
...
async function loadDirectorySnapshot(sdk: OpenResearchClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, openresearch provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const openresearchProvider = providers[ProviderV2.ID.make("openresearch")]
  const openresearchModel = openresearchProvider ? Provider.sort(Object.values(openresearchProvider.models))[0] : undefined
  if (openresearchProvider && openresearchModel) return { providerID: openresearchProvider.id, modelID: openresearchModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "OpenResearch prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "OpenResearch service failure", service })
}
