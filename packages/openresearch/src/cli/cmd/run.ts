import type { PermissionV1 } from "@openresearch-ai/core/v1/permission"
import { FSUtil } from "@openresearch-ai/core/fs-util"
// CLI entry point for `openresearch run` and `openresearch --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`openresearch --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`openresearch --mini --attach`): connects to a running
//      openresearch server and runs interactive mode against it.
//
...
import { createOpenResearchClient, type OpenResearchClient, type ToolPart } from "@openresearch-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<OpenResearchClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run openresearch with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running openresearch server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OPENRESEARCH_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENRESEARCH_SERVER_USERNAME or 'openresearch')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const attachSDK = (dir?: string) => {
        return createOpenResearchClient({
          baseUrl: args.attach!,
          directory: dir,
      ...
      async function session(sdk: OpenResearchClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: OpenResearchClient, sessionID: string) {
      ...
      async function current(sdk: OpenResearchClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: OpenResearchClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: OpenResearchClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: OpenResearchClient) {
        ...
        async function loop(client: OpenResearchClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createOpenResearchClient({
        baseUrl: "http://openresearch.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
