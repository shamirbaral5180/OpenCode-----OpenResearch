...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall openresearch and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenResearch")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g openresearch-ai",
      pnpm: "pnpm uninstall -g openresearch-ai",
      bun: "bun remove -g openresearch-ai",
      yarn: "yarn global remove openresearch-ai",
      brew: "brew uninstall openresearch",
      choco: "choco uninstall openresearch",
      scoop: "scoop uninstall openresearch",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "openresearch", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenResearch!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# openresearch") || content.includes(".openresearch/bin")) {
      return file
    }
  }
