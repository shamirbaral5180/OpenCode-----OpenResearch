...
import { IdeEvent } from "@openresearch-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["OPENRESEARCH_CALLER"] === "vscode" || process.env["OPENRESEARCH_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.openresearch"], {
    nothrow: true,
  })
