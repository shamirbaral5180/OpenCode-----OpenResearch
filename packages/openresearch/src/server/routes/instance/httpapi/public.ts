...
import { OpenResearchHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for openresearch serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(OpenResearchHttpApi)
...
export const PublicApi = OpenResearchHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "openresearch",
    version: "1.0.0",
    description: "openresearch api",
