...
import { AbsolutePath, type DeepMutable } from "@openresearch-ai/core/schema"
...
export class Service extends Context.Service<Service, Interface>()("@openresearch/Agent") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Agent.state")(function* (ctx) {
        ...
        const research = Permission.fromConfig({
          "*": "ask",
        ...
        const agents: Record<string, Info> = {
          research: {
            name: "research",
            description: "Deep research with traceable evidence, citations, and report-only writing.",
            prompt: ResearchPrompt.system,
            mode: "primary",
            native: true,
            options: {},
            permission: Permission.merge(defaults, research, user),
          },
          ...Object.fromEntries(
            [
              [
              ...
              name,
              {
                ...
                native: true,
                options: {},
                // Parent permissions do not inherit into task sessions.
                permission: Permission.merge(
                  defaults,
                  research,
                  Permission.fromConfig({ edit: "deny", task: "deny", todowrite: "deny" }),
                  user,
                ),
              },
            ]),
          ...
          build: {
            name: "build",
            hidden: true,
            description: "Legacy coding agent. Executes tools based on configured permissions.",
            options: {},
            permission: Permission.merge(
              defaults,
        ...
        const list = Effect.fnUntraced(function* () {
          const selected = yield* defaultInfo()
          return pipe(agents, values(), sortBy([(x) => x.name === selected.name, "desc"], [(x) => x.name, "asc"]))
        })
