...
const TUI_SCHEMA_URL = "https://openresearch.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from openresearch.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const openresearch = await openresearchFiles(input)
  for (const file of openresearch) {
...
async function openresearchFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "openresearch"),
    ...(await Filesystem.findUp(["openresearch.json", "openresearch.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "openresearch"))
  }
  if (Flag.OPENRESEARCH_CONFIG) files.push(Flag.OPENRESEARCH_CONFIG)
