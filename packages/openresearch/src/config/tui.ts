...
import { isRecord } from "@openresearch-ai/tui/util/record"
...
import { TuiKeybind } from "@openresearch-ai/tui/config/keybind"
...
import { TuiConfig } from "@openresearch-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@openresearch/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old openresearch.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.openresearch`
  // folders between cwd and home, and OPENRESEARCH_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.OPENRESEARCH_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit OPENRESEARCH_TUI_CONFIG override, if set.
  if (Flag.OPENRESEARCH_TUI_CONFIG) {
    const configFile = Flag.OPENRESEARCH_TUI_CONFIG
  ...
  // 4. `.openresearch` directories (and OPENRESEARCH_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".openresearch") || dir === Flag.OPENRESEARCH_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".openresearch") && dir !== Flag.OPENRESEARCH_CONFIG_DIR) continue
