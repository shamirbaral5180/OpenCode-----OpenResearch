...
import { NonNegativeInt, PositiveInt } from "@openresearch-ai/core/schema"
import { ConfigAttachmentV1 } from "@openresearch-ai/core/v1/config/attachment"
import { ConfigLSPV1 } from "@openresearch-ai/core/v1/config/lsp"
import { InvalidError } from "@openresearch-ai/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by OpenResearch V1. Use V1 "permission" rules or run openresearch2.',
      })),
    })
