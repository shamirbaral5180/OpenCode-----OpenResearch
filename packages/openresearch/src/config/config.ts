...
import { InstallationLocal, InstallationVersion } from "@openresearch-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@openresearch-ai/core/v1/config/console-state"
import { FSUtil } from "@openresearch-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@openresearch-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@openresearch-ai/core/v1/config/config"
import { RemoteAuthError } from "@openresearch-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@openresearch-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@openresearch/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["openresearch.json", "openresearch.jsonc", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://openresearch.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENRESEARCH_CONFIG && !Flag.OPENRESEARCH_CONFIG_DIR && !Flag.OPENRESEARCH_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://openresearch.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openresearch.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openresearch.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://openresearch.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "OPENRESEARCH_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/openresearch`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://openresearch.ai/config.json"
        ...
        if (Flag.OPENRESEARCH_CONFIG) {
          yield* merge(Flag.OPENRESEARCH_CONFIG, yield* loadFile(Flag.OPENRESEARCH_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.OPENRESEARCH_CONFIG })
        }

        if (!Flag.OPENRESEARCH_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("openresearch", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.OPENRESEARCH_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from OPENRESEARCH_CONFIG_DIR", { path: Flag.OPENRESEARCH_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".openresearch") || dir === Flag.OPENRESEARCH_CONFIG_DIR) {
            for (const file of ["openresearch.json", "openresearch.jsonc"]) {
          ...
          if (!Flag.OPENRESEARCH_DISABLE_PLUGIN_DEPENDENCY_INSTALL) {
            const dep = yield* npmSvc
              .install(dir, {
                add: [
                  {
                    name: "@openresearch-ai/plugin",
          ...
          // Auto-discovered plugins under `.openresearch/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.OPENRESEARCH_CONFIG_CONTENT) {
          const source = "OPENRESEARCH_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.OPENRESEARCH_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from OPENRESEARCH_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["OPENRESEARCH_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("OPENRESEARCH_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.OPENRESEARCH_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OPENRESEARCH_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("OPENRESEARCH_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.OPENRESEARCH_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.OPENRESEARCH_DISABLE_PRUNE) {
