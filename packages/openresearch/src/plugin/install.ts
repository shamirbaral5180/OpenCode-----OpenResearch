...
export type PatchDeps = {
  ...
  files: (dir: string, name: "openresearch" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".openresearch")
}

function patchName(kind: Kind): "openresearch" | "tui" {
  if (kind === "server") return "openresearch"
  return "tui"
}
