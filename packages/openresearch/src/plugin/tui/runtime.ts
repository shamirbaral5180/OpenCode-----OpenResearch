...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@openresearch-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@openresearch-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@openresearch-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@openresearch-ai/tui/context/theme"
import { Global } from "@openresearch-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@openresearch-ai/core/util/flock"
import { Flag } from "@openresearch-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@openresearch-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@openresearch-ai/core/v1/config/plugin"
import { createCommandShim } from "@openresearch-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@openresearch-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".openresearch"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".openresearch", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".openresearch", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.OPENRESEARCH_PURE ? [] : pluginOrigins
    if (Flag.OPENRESEARCH_PURE && pluginOrigins.length) {
    }
