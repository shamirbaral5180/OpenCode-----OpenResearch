...
export const DEPRECATED_PLUGIN_PACKAGES = ["openresearch-openai-codex-auth", "openresearch-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, openresearchVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(openresearchVersion) || semver.major(openresearchVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.openresearch
  if (typeof range !== "string") return
  if (!semver.satisfies(openresearchVersion, range)) {
    throw new Error(`Plugin requires openresearch ${range} but running ${openresearchVersion}`)
  }
}
