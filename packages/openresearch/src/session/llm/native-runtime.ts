...
import type { LLMClientShape } from "@openresearch-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("openresearch"))
    return { type: "unsupported", reason: "provider is not openai, openresearch, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @openresearch-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains openresearch-owned. The native runtime only adapts
      // the @openresearch-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
