...
This is the living design and status document for `@openresearch-ai/codemode` and its existing V2 OpenResearch adapter.
...
### V2 OpenResearch adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/openresearch/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
