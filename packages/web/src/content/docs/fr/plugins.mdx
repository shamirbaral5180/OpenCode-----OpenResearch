---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenResearch.
---

Les plugins vous permettent d'étendre OpenResearch en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenResearch.
...
- `.openresearch/plugins/` - Plugins au niveau du projet
- `~/.config/openresearch/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/openresearch/node_modules/`.
...
1. Configuration globale (`~/.config/openresearch/openresearch.json`)
2. Configuration du projet (`openresearch.json`)
3. Répertoire global des plugins (`~/.config/openresearch/plugins/`)
4. Répertoire des plugins du projet (`.openresearch/plugins/`)
...
OpenResearch exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK openresearch pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre openresearch.
...
:::note
Si vous utilisez l'application de bureau OpenResearch, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez openresearch de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à openresearch :
...
L'assistant `tool` crée un outil personnalisé que openresearch peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour openresearch aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://openresearch.ai/docs/sdk) pour plus de détails.
