---
title: Serveur
description: Interagissez avec le serveur openresearch via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `openresearch serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client openresearch peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `openresearch.local` |
...
Définissez `OPENRESEARCH_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `openresearch`, ou définissez `OPENRESEARCH_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `openresearch serve` et à `openresearch web`.
...
Lorsque vous exécutez `openresearch`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur openresearch pour interagir avec openresearch par programmation.
:::

Cette architecture permet à openresearch de prendre en charge plusieurs clients et vous permet d'interagir avec openresearch par programme.

Vous pouvez exécuter `openresearch serve` pour démarrer un serveur autonome. Si vous avez le
openresearch TUI en cours d'exécution, `openresearch serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenResearch [IDE](/docs/ide).
...
Le serveur openresearch expose les API suivantes.
