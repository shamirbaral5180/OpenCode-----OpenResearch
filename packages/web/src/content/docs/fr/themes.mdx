...
Avec OpenResearch, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenResearch utilise notre propre thème `openresearch`.
...
OpenResearch est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenResearch corresponde à l'apparence de leur terminal
...
OpenResearch prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/openresearch/themes/*.json` ou `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.openresearch/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.openresearch/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/openresearch/themes
vim ~/.config/openresearch/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .openresearch/themes
vim .openresearch/themes/my-theme.json
```
...
{
  "$schema": "https://openresearch.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
