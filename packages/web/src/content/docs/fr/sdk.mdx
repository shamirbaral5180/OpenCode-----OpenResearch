---
title: SDK
description: Client JS de type sécurisé pour le serveur openresearch.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK openresearch JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler openresearch par programme.
...
```bash
npm install @openresearch-ai/sdk
```
...
Créez une instance de openresearch :

```javascript
import { createOpenResearch } from "@openresearch-ai/sdk"

const { client } = await createOpenResearch()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `openresearch.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${openresearch.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de openresearch, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@openresearch-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*openresearch" },
})
