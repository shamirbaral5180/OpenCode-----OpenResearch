...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OpenResearch dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OpenResearch.
...
:::note
Ovaj alat je dostupan samo kada je `OPENRESEARCH_EXPERIMENTAL_LSP_TOOL=true` (ili `OPENRESEARCH_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenResearch provajdera ili kada je varijabla `OPENRESEARCH_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OpenResearch:
