...
U OpenResearch mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenResearch koristi nasu `openresearch` temu.
...
OpenResearch dolazi sa vise ugradenih tema.
...
- Zele da OpenResearch odgovara izgledu njihovog terminala
...
OpenResearch podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/openresearch/themes/*.json` ili `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.openresearch/themes/*.json`
4. **Trenutni radni direktorij** - `./.openresearch/themes/*.json`
