...
Postoje dvije vrste agenata u OpenResearch: primarni agenti i podagenti.
...
OpenResearch dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
OpenResearch dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

OpenResearch dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u OpenResearch-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `openresearch.json`:
...
- Globalno: `~/.config/openresearch/agents/`
- Po projektu: `.openresearch/agents/`

```markdown title="~/.config/openresearch/agents/review.md"
---
...
Ako temperatura nije navedena, OpenResearch koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu OpenResearch konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj OpenResearch konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenResearch Zen](/docs/zen), koristili biste `openresearch/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="openresearch.json" {3-6,9-12}
{
  "$schema": "https://openresearch.ai/config.json",
  "tools": {
    "write": true,
...
```json title="openresearch.json" {3-5,8-10}
{
  "$schema": "https://openresearch.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="openresearch.json" {8}
{
  "$schema": "https://openresearch.ai/config.json",
  "agent": {
    "build": {
...
```json title="openresearch.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Pokrenite `openresearch models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/openresearch).
:::
...
```markdown title="~/.config/openresearch/agents/docs-writer.md"
---
...
```markdown title="~/.config/openresearch/agents/security-auditor.md"
---
