---
title: Konfiguracija
description: Korištenje OpenResearch JSON konfiguracije.
---

Možete konfigurirati OpenResearch koristeći JSON konfiguracijski fajl.

---

## Format

OpenResearch podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/openresearch`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/openresearch/openresearch.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENRESEARCH_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`openresearch.json` u projektu) - postavke specifične za projekat
5. **`.openresearch` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OPENRESEARCH_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.openresearch` i `~/.config/openresearch` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/openresearch` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu OpenResearch konfiguraciju u `~/.config/openresearch/openresearch.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/openresearch/tui.json`.
...
Dodajte `openresearch.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se OpenResearch pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OPENRESEARCH_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENRESEARCH_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.openresearch` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.openresearch`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`openresearch.ai/config.json`**](https://openresearch.ai/config.json).

TUI konfiguracija koristi [**`openresearch.ai/tui.json`**](https://openresearch.ai/tui.json).
...
Koristite `OPENRESEARCH_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `openresearch.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `openresearch serve` i `openresearch web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš OpenResearch server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `openresearch.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj OpenResearch konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, OpenResearch pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/openresearch/agents/` ili `.openresearch/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, OpenResearch će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`openresearch run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/openresearch/commands/` ili `.openresearch/commands/`. [Saznajte više ovdje](/docs/commands).
...
OpenResearch će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, OpenResearch **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju OpenResearch sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.openresearch/plugins/` ili `~/.config/openresearch/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite OpenResearch da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
