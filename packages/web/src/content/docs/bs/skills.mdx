...
Agent skills omogucavaju OpenResearch da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
OpenResearch pretrazuje ove lokacije:

- Konfiguracija projekta: `.openresearch/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/openresearch/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, OpenResearch ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.openresearch/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/openresearch/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.openresearch/skills/git-release/SKILL.md` ovako:
...
OpenResearch navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `openresearch.json`:
...
**Za ugradene agente** (u `openresearch.json`):
