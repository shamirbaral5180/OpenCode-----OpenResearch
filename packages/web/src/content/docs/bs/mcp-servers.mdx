...
Možete dodati vanjske alate u OpenResearch koristeći _Model Context Protocol_, ili MCP. OpenResearch podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenResearch Config](https://openresearch.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/openresearch`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OpenResearch automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenResearch će:
...
Ako server zahtijeva autentifikaciju, OpenResearch će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `openresearch mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenResearch sigurno čuva tokene u `~/.local/share/openresearch/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenResearch, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenResearch konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenResearch sa vašim Sentry nalogom.
