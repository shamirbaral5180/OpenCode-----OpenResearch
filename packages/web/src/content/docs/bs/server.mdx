---
title: Server
description: Komunicirajte s openresearch serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `openresearch serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji openresearch klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `openresearch.local` |
...
Postavite `OPENRESEARCH_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `openresearch`, ili postavite `OPENRESEARCH_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `openresearch serve` i za `openresearch web`.
...
Kada pokrenete `openresearch`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite openresearch server za programsku interakciju sa openresearch-om.
:::

Ova arhitektura omogucava openresearch podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `openresearch serve` da startate standalone server. Ako je openresearch TUI vec pokrenut, `openresearch serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenResearch [IDE](/docs/ide) pluginovi.
...
openresearch server izlaže sljedece API-je.
