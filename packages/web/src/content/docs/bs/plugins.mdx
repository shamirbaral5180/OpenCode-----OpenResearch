---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenResearch.
---

Dodaci vam omogućavaju da proširite OpenResearch spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenResearch.
...
- `.openresearch/plugins/` - Dodaci na nivou projekta
- `~/.config/openresearch/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/openresearch/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/openresearch/openresearch.json`)
2. Konfiguracija projekta (`openresearch.json`)
3. Globalni direktorij dodataka (`~/.config/openresearch/plugins/`)
4. Direktorij dodataka projekta (`.openresearch/plugins/`)
...
OpenResearch pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: OpenResearch SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenResearch.
...
Spriječite openresearch da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u openresearch:
...
Pomoćnik `tool` kreira prilagođeni alat koji openresearch može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za openresearch zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://openresearch.ai/docs/sdk) za detalje.
