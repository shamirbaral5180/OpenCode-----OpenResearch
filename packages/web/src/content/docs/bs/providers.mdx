---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenResearch.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenResearch koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenResearch konfiguraciji.
...
u `~/.local/share/openresearch/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenResearch
config.
...
OpenResearch Zen je lista modela koje je obezbedio OpenResearch tim koji su bili
testirano i potvrđeno da dobro radi sa OpenResearch. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenResearch Zen.
:::
...
Radi kao i svaki drugi provajder u OpenResearch i potpuno je opcionalan za korištenje.

---

## OpenResearch Go

OpenResearch Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OpenResearch tim i koji su testirani i verificirani da dobro rade s OpenResearch-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenResearch Go` i idite na [openresearch.ai/auth](https://openresearch.ai/zen).
...
Da biste koristili Amazon Bedrock s OpenResearch:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete openresearch:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `openresearch.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenResearch.
:::
...
Možete konfigurirati openresearch za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi openresearch ispravno radio.
   :::
...
Također možete dodati modele kroz svoju openresearch konfiguraciju.
...
3.  Naziv: `OpenResearch`, opseg: `api`
...
da pohrani token u openresearch auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenResearch koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenResearch
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`openresearch.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [openresearch-gitlab-auth](https://www.npmjs.com/package/openresearch-gitlab-auth).

##### Konfiguracija

Prilagodite putem `openresearch.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s openresearch:
...
Za korištenje Google Vertex AI s OpenResearch:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće openresearch.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem openresearcha, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [openresearch-helicone-session](https://github.com/H2Shami/openresearch-helicone-session) da automatski prijavite svaki OpenResearch razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenResearch razgovor naveden kao zasebna sesija.
...
Možete konfigurirati openresearch za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati openresearch za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati openresearch za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenResearch. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/openresearch) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenResearch:
...
3. Kopirajte API ključ za korištenje u OpenResearch.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenResearch, morate lokalno povući informacije o modelu:
...
OpenResearch Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenResearch tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenResearch Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenResearch Zen**.

```txt
   /connect
```

3. Unesite svoj OpenResearch API ključ.
...
Također možete dodati dodatne modele putem vaše openresearch konfiguracije.
...
5. Također ih možete prilagoditi putem vaše openresearch konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa OpenResearchom:
...
Također možete prilagoditi modele kroz svoju openresearch konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s openresearch-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `openresearch.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenResearch da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `openresearch auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenResearch konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj openresearch konfiguraciji.
