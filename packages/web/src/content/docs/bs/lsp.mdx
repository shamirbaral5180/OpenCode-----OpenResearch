---
title: LSP serveri
description: OpenResearch se integriše sa vašim LSP serverima.
---

OpenResearch se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

OpenResearch dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `OPENRESEARCH_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i openresearch otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj openresearch konfiguraciji.
