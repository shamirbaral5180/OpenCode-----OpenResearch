...
Mit OpenResearch kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet OpenResearch das `openresearch`-Theme.
...
OpenResearch liefert mehrere eingebaute Themes mit.
...
- OpenResearch optisch ans Terminal anpassen wollen
...
OpenResearch unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/openresearch/themes/*.json` oder `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.openresearch/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.openresearch/themes/*.json`
