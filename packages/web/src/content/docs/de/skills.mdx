...
Agent Skills erlauben OpenResearch, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenResearch sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenResearch vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.openresearch/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/openresearch/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.openresearch/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenResearch listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `openresearch.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `openresearch.json`):
