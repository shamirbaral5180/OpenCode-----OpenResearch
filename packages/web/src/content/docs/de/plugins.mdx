---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenResearch zu erweitern.
---

Mit Plugins können Sie OpenResearch erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenResearch zu ändern.
...
- `.openresearch/plugins/` – Plugins auf Projektebene
- `~/.config/openresearch/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/openresearch/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/openresearch/openresearch.json`)
2. Projektkonfiguration (`openresearch.json`)
3. Globales Plugin-Verzeichnis (`~/.config/openresearch/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.openresearch/plugins/`)
...
OpenResearch führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenResearch-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenResearch erweitern können.
...
:::note
Wenn Sie die OpenResearch-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenResearch `.env`-Dateien liest:
...
Plugins können OpenResearch auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenResearch aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenResearch zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://openresearch.ai/docs/sdk).
