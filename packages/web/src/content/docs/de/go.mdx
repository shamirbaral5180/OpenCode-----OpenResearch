...
OpenResearch Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in OpenResearch. Du abonnierst OpenResearch Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
OpenResearch zu verwenden.
...
OpenResearch Go bietet dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

OpenResearch Go funktioniert wie jeder andere Provider in OpenResearch.

1. Du meldest dich bei **<a href={console}>OpenResearch Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `OpenResearch Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann OpenResearch Go abonnieren.
:::
...
OpenResearch Go wurde für [OpenResearch](https://openresearch.ai) und andere Coding-Agenten entwickelt,
...
3. für jede Unterhaltung eine stabile Sitzungs-ID in `x-openresearch-session` senden, damit wir das Routing und
   Prompt-Caching optimieren können.

### Validierte Clients

Neben OpenResearch wurden die folgenden Clients für die ordnungsgemäße Verwendung mit
OpenResearch Go validiert. Wir garantieren jedoch nicht, dass sie auch in Zukunft funktionieren werden.

| Client            | Sitzungsunterstützung                                                                                                                                                                                                                                         |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Builds mit [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) senden den Header bei Haupt- und zusätzlichen OpenResearch-Anfragen. Der Fix wurde nach v0.21.0 gemergt; diese Version allein enthält ihn nicht.                                |
...
| **ZCode**         | Go erkennt seinen nativen Sitzungs-Header. Unsere [Anfrage für `x-openresearch-session`](https://github.com/zai-org/feedback/issues/492) ist weiterhin offen, aber es ist nicht mehr erforderlich, genau diesen Header zu senden.                                 |
| **Pi**            | Aktuelle Builds senden Sitzungsinformationen für OpenResearch. Aktualisiere ältere Installationen.                                                                                                                                                                |
...
| **Kilo Code CLI** | Builds mit [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) stellen OpenResearch-Sitzungs-Header wieder her. Dieser Fix gilt für die CLI, nicht für die VS-Code-Erweiterung. Siehe [Issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
Die [Modell-ID](/docs/config/#models) in deiner OpenResearch Config verwendet das Format `openresearch-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `openresearch-go/kimi-k3` in deiner Config verwenden.
...
Wir haben OpenResearch Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit OpenResearch zu nutzen.
