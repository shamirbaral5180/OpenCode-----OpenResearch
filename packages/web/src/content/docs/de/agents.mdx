...
Es gibt zwei Arten von Agenten in OpenResearch; Primäragenten und Subagenten.
...
OpenResearch verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `openresearch.json`-Konfigurationsdatei:
...
- Pro Projekt: `.openresearch/agents/`
...
Wenn keine Temperatur angegeben ist, verwendet OpenResearch modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale OpenResearch-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer OpenResearch-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [OpenResearch Zen](/docs/zen) verwenden, würden Sie `openresearch/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `openresearch models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/openresearch).
:::
