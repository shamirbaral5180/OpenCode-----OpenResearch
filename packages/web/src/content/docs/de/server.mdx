---
title: Server
description: Nutze den openresearch-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `openresearch serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein openresearch-Client nutzen kann.
...
Setze `OPENRESEARCH_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `openresearch`, kann aber mit `OPENRESEARCH_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `openresearch serve` und `openresearch web`.
...
Wenn du `openresearch` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den openresearch-Server, um openresearch programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `openresearch serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `openresearch serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die OpenResearch-[IDE](/docs/ide)-Plugins.
...
Der openresearch-Server stellt folgende APIs bereit.
