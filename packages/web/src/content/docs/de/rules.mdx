---
title: Regeln
description: Lege eigene Anweisungen fuer openresearch fest.
---

Du kannst openresearch ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in openresearch den Befehl `/init` aus.
...
So kann openresearch sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht openresearch sie zu erweitern.
...
openresearch kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/openresearch/AGENTS.md` speichern.
Diese gelten in allen openresearch-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenResearch dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/openresearch/AGENTS.md` existiert)
...
Beim Start sucht openresearch in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/openresearch/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `openresearch.json` oder in `~/.config/openresearch/openresearch.json` angeben.
...
openresearch parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit openresearch.json

Empfohlen ist das `instructions`-Feld in der `openresearch.json`:
...
Du kannst openresearch explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `openresearch.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
