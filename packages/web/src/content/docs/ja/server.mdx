---
title: サーバー
description: HTTP 経由で OpenResearch サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`openresearch serve` コマンドは、OpenResearch クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `openresearch.local` |
...
HTTP 基本認証でサーバーを保護するには、`OPENRESEARCH_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `openresearch` になるか、`OPENRESEARCH_SERVER_USERNAME` を設定してオーバーライドします。これは、`openresearch serve` と `openresearch web` の両方に当てはまります。
...
`openresearch` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
OpenResearch サーバーを使用して、プログラムで OpenResearch と対話します。
:::

`openresearch serve` を実行してスタンドアロンサーバーを起動できます。すでに OpenResearch TUI を実行している場合でも、`openresearch serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、OpenResearch [IDE](/docs/ide) プラグインによって使用されます。
...
OpenResearch サーバーは次の API を公開します。
