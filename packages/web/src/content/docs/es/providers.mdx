---
title: Proveedores
description: Usando cualquier proveedor LLM en OpenResearch.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenResearch usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración OpenResearch.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu OpenResearch
configuración.
...
OpenResearch Zen es una lista de modelos proporcionados por el equipo OpenResearch que han sido
probado y verificado para funcionar bien con OpenResearch. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con OpenResearch Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione openresearch y diríjase a [openresearch.ai/auth](https://openresearch.ai/auth).
...
Funciona como cualquier otro proveedor en OpenResearch y su uso es completamente opcional.

---

## OpenResearch Go

OpenResearch Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de OpenResearch que han sido
probado y verificado para funcionar bien con OpenResearch.

1. Ejecute el comando `/connect` en TUI, seleccione `OpenResearch Go` y diríjase a [openresearch.ai/auth](https://openresearch.ai/zen).
...
Para usar Amazon Bedrock con OpenResearch:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta openresearch:
   ...
   Para una configuración persistente o específica del proyecto, utilice `openresearch.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en OpenResearch no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar openresearch para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que openresearch funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración openresearch.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `OpenResearch`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación openresearch.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
OpenResearch utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear OpenResearch
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `openresearch.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [openresearch-gitlab-auth](https://www.npmjs.com/package/openresearch-gitlab-auth).

##### Configuración

Personalizar a través de `openresearch.json`:
...
Para utilizar su suscripción GitHub Copilot con openresearch:
...
Para utilizar Google Vertex AI con OpenResearch:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta openresearch.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de openresearch, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [openresearch-helicone-session](https://github.com/H2Shami/openresearch-helicone-session) para registrar automáticamente cada conversación OpenResearch como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación OpenResearch enumerada como una sesión separada.
...
Puede configurar openresearch para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar openresearch para usar modelos locales a través de LM Studio.
...
Puedes configurar openresearch para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para OpenResearch. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/openresearch) para obtener más detalles.
:::
...
Para usar Ollama Cloud con OpenResearch:
...
3. Copie la clave API para usarla en OpenResearch.
...
6. **Importante**: Antes de usar modelos en la nube en OpenResearch, debe obtener la información del modelo localmente:
...
OpenResearch Zen es una lista de modelos probados y verificados proporcionada por el equipo OpenResearch. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>OpenResearch Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **OpenResearch Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave OpenResearch API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración openresearch.
...
5. También puedes personalizarlos a través de tu configuración openresearch. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con OpenResearch:
...
También puede personalizar modelos a través de su configuración openresearch. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con openresearch. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `openresearch.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a OpenResearch comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `openresearch auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración openresearch y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración openresearch.
