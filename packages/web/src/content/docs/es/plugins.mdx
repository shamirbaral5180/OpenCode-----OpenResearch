---
title: Complementos
description: Escriba sus propios complementos para extender OpenResearch.
---

Los complementos le permiten extender OpenResearch al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OpenResearch.
...
- `.openresearch/plugins/` - Complementos a nivel de proyecto
- `~/.config/openresearch/plugins/` - Complementos globales
...
```json title="openresearch.json"
{
  ...
  "plugin": ["openresearch-helicone-session", "openresearch-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/openresearch/node_modules/`.
...
1. Configuración global (`~/.config/openresearch/openresearch.json`)
2. Configuración del proyecto (`openresearch.json`)
3. Directorio global de complementos (`~/.config/openresearch/plugins/`)
4. Directorio de complementos del proyecto (`.openresearch/plugins/`)
...
```json title=".openresearch/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
OpenResearch ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".openresearch/plugins/my-plugin.ts"
...
```js title=".openresearch/plugins/example.js"
...
- `client`: Un cliente SDK openresearch para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar openresearch.
...
```js title=".openresearch/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "openresearch"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio OpenResearch, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que openresearch lea archivos `.env`:

```javascript title=".openresearch/plugins/env-protection.js"
...
```javascript title=".openresearch/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a openresearch:

```ts title=".openresearch/plugins/custom-tools.ts"
import { type Plugin, tool } from "@openresearch-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que openresearch puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para openresearch junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://openresearch.ai/docs/sdk) para obtener más detalles.
...
```ts title=".openresearch/plugins/compaction.ts"
import type { Plugin } from "@openresearch-ai/plugin"
...
```ts title=".openresearch/plugins/custom-compaction.ts"
import type { Plugin } from "@openresearch-ai/plugin"
