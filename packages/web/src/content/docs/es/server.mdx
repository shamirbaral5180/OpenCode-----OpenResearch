---
title: Servidor
description: Interactuar con el servidor openresearch a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `openresearch serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente openresearch puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `openresearch.local` |
...
Configure `OPENRESEARCH_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `openresearch`, o configure `OPENRESEARCH_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `openresearch serve` como a `openresearch web`.
...
Cuando ejecuta `openresearch`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor openresearch para interactuar con openresearch mediante programación.
:::

Esta arquitectura permite que openresearch admita múltiples clientes y le permite interactuar con openresearch mediante programación.

Puede ejecutar `openresearch serve` para iniciar un servidor independiente. Si tienes el
openresearch TUI ejecutándose, `openresearch serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenResearch [IDE](/docs/ide).
...
El servidor openresearch expone las siguientes API.
