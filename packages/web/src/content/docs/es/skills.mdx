...
Las habilidades del agente permiten a OpenResearch descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OpenResearch busca estas ubicaciones:

- Configuración del proyecto: `.openresearch/skills/<name>/SKILL.md`
- Configuración global: `~/.config/openresearch/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OpenResearch sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.openresearch/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/openresearch/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.openresearch/skills/git-release/SKILL.md` así:
...
OpenResearch enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `openresearch.json`:
...
**Para agentes integrados** (en `openresearch.json`):
