...
description: Cree herramientas que LLM pueda llamar en openresearch.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de openresearch como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.openresearch/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/openresearch/tools/`.
...
```ts title=".openresearch/tools/database.ts" {1}
...
```ts title=".openresearch/tools/math.ts"
import { tool } from "@openresearch-ai/plugin"
...
```ts title=".openresearch/tools/bash.ts"
import { tool } from "@openresearch-ai/plugin"
...
```ts title=".openresearch/tools/project.ts" {8}
import { tool } from "@openresearch-ai/plugin"
...
```python title=".openresearch/tools/add.py"
import sys
...
```ts title=".openresearch/tools/python-add.ts" {10}
import { tool } from "@openresearch-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".openresearch/tools/add.py")
