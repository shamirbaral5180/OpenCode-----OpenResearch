...
Para depurar problemas con OpenResearch, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/openresearch/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openresearch\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `openresearch --log-level DEBUG`.

---

## Almacenamiento

openresearch almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/openresearch/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openresearch`
...
OpenResearch Desktop ejecuta un servidor OpenResearch local (el sidecar `openresearch-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `OpenResearch` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/openresearch/openresearch.jsonc` (o `~/.config/openresearch/openresearch.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/openresearch/openresearch.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openresearch\openresearch.jsonc`
...
OpenResearch también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/openresearch/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openresearch\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.openresearch/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que OpenResearch pueda reconstruirlo.

1. Salga de OpenResearch Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/openresearch`
- **Linux**: eliminar `~/.cache/openresearch` (o ejecutar `rm -rf ~/.cache/openresearch`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\openresearch`

3. Reinicie el escritorio OpenResearch.

---

### Solucionar problemas de conexión del servidor

OpenResearch Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `openresearch.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `OPENRESEARCH_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `OPENRESEARCH_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio OpenResearch requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de OpenResearch.

---

### Notificaciones que no se muestran

OpenResearch Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para OpenResearch en la configuración de su sistema operativo, y
...
1. Salga del escritorio OpenResearch.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio OpenResearch):

- `openresearch.settings.dat` (URL del servidor predeterminado de escritorio)
- `openresearch.global.dat` y `openresearch.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con OpenResearch:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/openresearch/issues**](https://github.com/anomalyco/openresearch/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**openresearch.ai/discord**](https://openresearch.ai/discord)
...
3. Asegúrese de tener la última versión con `openresearch upgrade`
...
- `openresearch/kimi-k2`

Para saber a qué modelos tiene acceso, ejecute `openresearch models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/openresearch
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\openresearch`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. openresearch instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/openresearch
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\openresearch`

2. Reinicie openresearch para reinstalar los paquetes del proveedor más recientes.

Esto obligará a openresearch a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
openresearch detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
