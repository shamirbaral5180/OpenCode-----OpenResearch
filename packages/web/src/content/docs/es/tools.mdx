...
Las herramientas permiten que LLM realice acciones en su código base. OpenResearch viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en OpenResearch.
...
```json title="openresearch.json" {4}
{
  "$schema": "https://openresearch.ai/config.json",
  "permission": {
    "bash": "allow"
...
:::note
Esta herramienta solo está disponible cuando `OPENRESEARCH_EXPERIMENTAL_LSP_TOOL=true` (o `OPENRESEARCH_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenResearch o cuando la variable de entorno `OPENRESEARCH_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenResearch:

```bash
OPENRESEARCH_ENABLE_EXA=1 openresearch
```
