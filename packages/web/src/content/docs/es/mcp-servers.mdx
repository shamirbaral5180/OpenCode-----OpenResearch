...
Puede agregar herramientas externas a OpenResearch utilizando el _Model Context Protocol_ o MCP. OpenResearch admite servidores locales y remotos.
...
Puede definir servidores MCP en su [OpenResearch Config](https://openresearch.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/openresearch`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
OpenResearch maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, OpenResearch:
...
Si el servidor requiere autenticación, OpenResearch le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `openresearch mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, OpenResearch almacenará los tokens de forma segura en `~/.local/share/openresearch/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en OpenResearch, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración OpenResearch como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar OpenResearch a su cuenta Sentry.
