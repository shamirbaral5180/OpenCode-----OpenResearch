...
Con OpenResearch puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, OpenResearch usa nuestro propio tema `openresearch`.
...
OpenResearch viene con varios temas integrados.
...
- Quiere que OpenResearch coincida con la apariencia de su terminal
...
OpenResearch admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/openresearch/themes/*.json` o `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.openresearch/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.openresearch/themes/*.json`
