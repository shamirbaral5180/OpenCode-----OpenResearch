---
title: TUI
description: Using the OpenResearch terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenResearch provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenResearch starts the TUI for the current directory.
...
When using the OpenResearch TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenResearch. Allows you to select from available providers and add their API keys.
...
Exit OpenResearch. _Aliases_: `/quit`, `/q`
...
This is separate from `openresearch.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long OpenResearch waits after the leader key. Defaults to `2000`.
...
Use `OPENRESEARCH_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `openresearch.default`.
