---
title: Tjener
description: Samhandle med OpenResearch-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openresearch serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenResearch-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `openresearch.local` |
...
Sett `OPENRESEARCH_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `openresearch` som standard, eller sett `OPENRESEARCH_SERVER_USERNAME` for å overstyre det. Dette gjelder både `openresearch serve` og `openresearch web`.
...
Når du kjører `openresearch` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OpenResearch-serveren til å samhandle med OpenResearch programmatisk.
:::

Denne arkitekturen lar OpenResearch støtte flere klienter og lar deg samhandle med OpenResearch programmatisk.

Du kan kjøre `openresearch serve` for å starte en frittstående server. Hvis du har
OpenResearch TUI kjører, vil `openresearch serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OpenResearch [IDE](/docs/ide) plugins.
...
openresearch-serveren viser følgende APIer.
