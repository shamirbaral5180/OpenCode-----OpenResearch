...
Du kan legge til eksterne verktøy i OpenResearch ved å bruke _Model Context Protocol_ eller MCP. OpenResearch støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenResearch Config](https://openresearch.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="openresearch.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/openresearch`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="openresearch.jsonc" {15}
{
  "$schema": "https://openresearch.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
OpenResearch håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenResearch:
...
Hvis serveren krever autentisering, vil OpenResearch be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `openresearch mcp auth <server-name>`.
...
```json title="openresearch.json" {7-11}
{
  "$schema": "https://openresearch.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
openresearch mcp auth my-oauth-server
```
...
```bash
openresearch mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenResearch lagre tokens sikkert i `~/.local/share/openresearch/mcp-auth.json`.
...
```json title="openresearch.json" {7}
{
  "$schema": "https://openresearch.ai/config.json",
  "mcp": {
    "my-api-key-server": {
...
openresearch mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i OpenResearch, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenResearch-konfigurasjonen som alle andre verktøy.
...
```json title="openresearch.json" {14}
{
  "$schema": "https://openresearch.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="openresearch.json" {11, 14-18}
{
  "$schema": "https://openresearch.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="openresearch.json" {4-8}
{
  "$schema": "https://openresearch.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
openresearch mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenResearch til Sentry-kontoen din.
...
```json title="openresearch.json" {4-7}
{
  "$schema": "https://openresearch.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="openresearch.json" {7-9}
{
  "$schema": "https://openresearch.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenResearch-samtale oppført som en separat økt.
