---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OpenResearch.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenResearch bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OpenResearch-konfigurasjonen.
...
i `~/.local/share/openresearch/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OpenResearch
konfig.
...
OpenResearch Zen er en liste over modeller levert av OpenResearch-teamet som har vært
testet og verifisert for å fungere godt med OpenResearch. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenResearch Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OpenResearch Zen` og gå til [openresearch.ai/auth](https://openresearch.ai/zen).
...
Det fungerer som alle andre leverandører i OpenResearch og er helt valgfritt å bruke.

---

## OpenResearch Go

OpenResearch Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av OpenResearch-teamet som har vært
testet og verifisert for å fungere godt med OpenResearch.

1. Kjør kommandoen `/connect` i TUI, velg `OpenResearch Go`, og gå til [openresearch.ai/auth](https://openresearch.ai/zen).
...
Slik bruker du Amazon Bedrock med OpenResearch:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OpenResearch:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `openresearch.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i openresearch støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere openresearch til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OpenResearch skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OpenResearch-konfigurasjonen.
...
for å lagre token i OpenResearch auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OpenResearch bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OpenResearch
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`openresearch.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [openresearch-gitlab-auth](https://www.npmjs.com/package/openresearch-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `openresearch.json`:
...
Slik bruker du GitHub Copilot-abonnementet med OpenResearch:
...
Slik bruker du Google Vertex AI med OpenResearch:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OpenResearch.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OpenResearch, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [openresearch-helicone-session](https://github.com/H2Shami/openresearch-helicone-session) plugin for automatisk å logge hver OpenResearch-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OpenResearch-samtale oppført som en separat økt.
...
Du kan konfigurere OpenResearch for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OpenResearch for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OpenResearch for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OpenResearch. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/openresearch) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OpenResearch:
...
3. Kopier API-nøkkelen for bruk i OpenResearch.
...
6. **Viktig**: Før du bruker skymodeller i OpenResearch, må du hente modellinformasjonen lokalt:
...
OpenResearch Zen er en liste over testede og verifiserte modeller levert av OpenResearch-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OpenResearch Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OpenResearch Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OpenResearch API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OpenResearch-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OpenResearch-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenResearch:
...
Du kan også tilpasse modeller gjennom OpenResearch-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OpenResearch. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `openresearch.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OpenResearch forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `openresearch auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OpenResearch-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OpenResearch-konfigurasjonen.
