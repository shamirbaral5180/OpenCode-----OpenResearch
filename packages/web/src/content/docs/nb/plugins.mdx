...
description: Skriv dine egne programtillegg for å utvide OpenResearch.
---

Plugins lar deg utvide OpenResearch ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenResearch.
...
- `.openresearch/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/openresearch/node_modules/`.
...
1. Global konfigurasjon (`~/.config/openresearch/openresearch.json`)
2. Prosjektkonfigurasjon (`openresearch.json`)
3. Global plugin-katalog (`~/.config/openresearch/plugins/`)
4. Prosjektpluginkatalog (`.openresearch/plugins/`)
...
OpenResearch kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenResearch SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenResearch.
...
:::note
Hvis du bruker OpenResearch-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenResearch fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenResearch:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenResearch kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenResearch sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://openresearch.ai/docs/sdk) for detaljer.
