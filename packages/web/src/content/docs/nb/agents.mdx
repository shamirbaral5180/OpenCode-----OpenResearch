...
Det er to typer agenter i OpenResearch; primære agenter og underagenter.
...
OpenResearch kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenResearch kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenResearch kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `openresearch.json`:
...
- Per prosjekt: `.openresearch/agents/`
...
Hvis ingen temperatur er spesifisert, bruker OpenResearch modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenResearch-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenResearch-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenResearch Zen](/docs/zen), vil du bruke `openresearch/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `openresearch models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/openresearch).
:::
