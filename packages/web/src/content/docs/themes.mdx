...
With OpenResearch you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenResearch uses our own `openresearch` theme.
...
OpenResearch comes with several built-in themes.
...
- Want OpenResearch to match their terminal's appearance
...
OpenResearch supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/openresearch/themes/*.json` or `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.openresearch/themes/*.json`
4. **Current working directory** - Defined in `./.openresearch/themes/*.json`
