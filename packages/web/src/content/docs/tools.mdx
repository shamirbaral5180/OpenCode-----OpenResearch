...
Tools allow the LLM to perform actions in your codebase. OpenResearch comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in OpenResearch.
...
:::note
This tool is only available when `OPENRESEARCH_EXPERIMENTAL_LSP_TOOL=true` (or `OPENRESEARCH_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the OpenResearch or OpenResearch Go provider, or when either the `OPENRESEARCH_ENABLE_EXA` or `OPENRESEARCH_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching OpenResearch:

```bash
OPENRESEARCH_ENABLE_EXA=1 openresearch
# or
OPENRESEARCH_ENABLE_PARALLEL=1 openresearch
```
