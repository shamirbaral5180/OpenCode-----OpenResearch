---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenResearch.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenResearch bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenResearch-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/openresearch/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OpenResearch-konfigurationen.
...
OpenResearch Zen er en liste over modeller leveret af OpenResearch-teamet, som er blevet testet og verificeret til at fungere godt med OpenResearch. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenResearch Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg openresearch og gå til [openresearch.ai/auth](https://openresearch.ai/auth).
...
Det fungerer som alle andre udbydere i OpenResearch og er helt valgfrit at bruge.

---

## OpenResearch Go

OpenResearch Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OpenResearch-teamet, som er testet og verificeret til at fungere godt med OpenResearch.

1. Kør kommandoen `/connect` i TUI, vælg `OpenResearch Go`, og gå til [openresearch.ai/auth](https://openresearch.ai/zen).
...
Sådan bruger du Amazon Bedrock med OpenResearch:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører openresearch:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `openresearch.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenResearch understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere openresearch til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at openresearch skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem openresearch-konfigurationen.
...
2. Vælg din godkendelsesmetode:
   ...
   3. Navn: `OpenResearch`, omfang: `api`
...
gemme tokenet i openresearch auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OpenResearch bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenResearch
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`openresearch.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [openresearch-gitlab-auth](https://www.npmjs.com/package/openresearch-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `openresearch.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med OpenResearch:
...
Sådan bruger du Google Vertex AI med OpenResearch:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører openresearch.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem openresearch, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [openresearch-helicone-session](https://github.com/H2Shami/openresearch-helicone-session) plugin for automatisk at logge hver OpenResearch-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenResearch-samtale opført som en separat session.
...
Du kan konfigurere openresearch til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere openresearch til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere openresearch til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenResearch. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/openresearch) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenResearch:
...
3. Kopier API-nøglen til brug i OpenResearch.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenResearch, skal du hente modelinformationen lokalt:
...
OpenResearch Zen er en liste over testede og verificerede modeller leveret af OpenResearch-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenResearch Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenResearch Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenResearch API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem openresearch-konfigurationen.
...
5. Du kan også tilpasse dem gennem openresearch-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenResearch:
...
Du kan også tilpasse modeller gennem openresearch-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med openresearch. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `openresearch.json`-filen i projektkataloget:
...
`limit`-felterne lader OpenResearch forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `openresearch auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek openresearch-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i openresearch-konfigurationen.
