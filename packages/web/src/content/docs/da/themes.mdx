...
Med OpenResearch kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OpenResearch vårt eget `openresearch`-tema.
...
OpenResearch kommer med flere innebygde temaer.
...
- Vil at OpenResearch skal matche terminalens utseende
...
OpenResearch støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/openresearch/themes/*.json` eller `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openresearch/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.openresearch/themes/*.json`
