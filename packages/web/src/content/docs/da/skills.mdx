...
Agentferdigheter lar OpenResearch oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
OpenResearch søker etter disse stedene:

- Prosjektkonfigurasjon: `.openresearch/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/openresearch/skills/<name>/SKILL.md`
...
For prosjektlokale stier går OpenResearch opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.openresearch/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/openresearch/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.openresearch/skills/git-release/SKILL.md` slik:
...
OpenResearch viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `openresearch.json`:
...
**For innebygde agenter** (i `openresearch.json`):
