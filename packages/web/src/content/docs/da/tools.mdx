...
Her er alle de innebygde verktøyene som er tilgængelige i OpenResearch.
...
:::note
Dette verktøyet er bare tilgængelig når `OPENRESEARCH_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENRESEARCH_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenResearch-leverandøren eller når miljøvariabelen `OPENRESEARCH_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
