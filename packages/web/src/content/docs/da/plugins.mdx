---
title: Plugins
description: Skriv dine egne plugins for at udvide OpenResearch.
---

Plugins giver dig mulighed for at udvide OpenResearch ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenResearchs standardadfærd.
...
- `.openresearch/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/openresearch/node_modules/`.
...
1. Global konfiguration (`~/.config/openresearch/openresearch.json`)
2. Projektkonfiguration (`openresearch.json`)
3. Global plugin-mappe (`~/.config/openresearch/plugins/`)
4. Projekt plugin bibliotek (`.openresearch/plugins/`)
...
OpenResearch kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En openresearch SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide openresearch.
...
:::note
Hvis du bruger OpenResearch desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå openresearch i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til openresearch:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som openresearch kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for openresearch sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://openresearch.ai/docs/sdk) for detaljer.
