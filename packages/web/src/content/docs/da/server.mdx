---
title: Server
description: Interager med openresearch-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openresearch serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en openresearch-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `openresearch.local` |
...
Indstil `OPENRESEARCH_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `openresearch`, eller indstil `OPENRESEARCH_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `openresearch serve` og `openresearch web`.
...
Når du kører `openresearch` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug openresearch-serveren til at interagere med openresearch programmatisk.
:::

Denne arkitektur lader openresearch understøtte flere klienter og lader dig interagere med openresearch programmatisk.

Du kan køre `openresearch serve` for at starte en selvstændig server. Hvis du har
openresearch TUI kørende, vil `openresearch serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OpenResearch [IDE](/docs/ide) plugins.
...
OpenResearch-serveren viser følgende API'er.
