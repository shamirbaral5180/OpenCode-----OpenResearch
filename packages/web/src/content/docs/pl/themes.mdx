...
Dzięki openresearch możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie openresearch używa naszego własnego motywu `openresearch`.
...
openresearch ma kilka wbudowanych motywów.
...
- Chcą, aby OpenResearch pasował do wyglądu ich terminala
...
openresearch obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/openresearch/themes/*.json` lub `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.openresearch/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.openresearch/themes/*.json`
