---
title: Konfiguracja
description: Ustawienia OpenResearch w pliku JSON.
---

Możesz dostosować OpenResearch za pomocą pliku konfiguracyjnego JSON.

---

## Format

OpenResearch obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "openresearch"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/openresearch`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/openresearch/openresearch.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENRESEARCH_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`openresearch.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.openresearch`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `OPENRESEARCH_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.openresearch` i `~/.config/openresearch` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/openresearch`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację OpenResearch w `~/.config/openresearch/openresearch.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/openresearch/tui.json`.
...
Dodaj `openresearch.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy OpenResearch się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `OPENRESEARCH_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENRESEARCH_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.openresearch` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.openresearch`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`openresearch.ai/config.json`**](https://openresearch.ai/config.json).
...
Użyj `OPENRESEARCH_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `openresearch.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `openresearch serve` i `openresearch web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer OpenResearch.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `openresearch.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie OpenResearch wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/openresearch/agents/` lub `.openresearch/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, OpenResearch powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`openresearch run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/openresearch/commands/` lub `.openresearch/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
OpenResearch może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy OpenResearch nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie OpenResearch **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają OpenResearch o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.openresearch/plugins/` lub `~/.config/openresearch/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie OpenResearch tylko do określonych dostawców.
