---
title: Serwery LSP
description: OpenResearch integruje się z serwerami LSP.
---

OpenResearch może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

OpenResearch posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `OPENRESEARCH_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i OpenResearch otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji OpenResearch.
