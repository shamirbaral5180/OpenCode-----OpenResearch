...
Umiejętności agenta pozwalają openresearch odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
openresearch przeszukuje te lokalizacje:

- Project config: `.openresearch/skills/<name>/SKILL.md`
- Global config: `~/.config/openresearch/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu openresearch przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.openresearch/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/openresearch/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.openresearch/skills/git-release/SKILL.md` w ten sposób:
...
openresearch wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `openresearch.json`:
...
**Dla agentów wbudowanych** (w `openresearch.json`):
