---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć openresearch.
---

Wtyczki umożliwiające wyłączenie openresearch poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania openresearch.
...
- `.openresearch/plugins/` - Wtyczki na poziomie projektu
- `~/.config/openresearch/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/openresearch/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/openresearch/openresearch.json`)
2. Konfiguracja projektu (`openresearch.json`)
3. Globalny katalog wtyczek (`~/.config/openresearch/plugins/`)
4. Katalog wtyczek projektu (`.openresearch/plugins/`)
...
openresearch uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z openresearch do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do openresearch.
...
:::note
Jeśli korzystasz z aplikacji komputerowej openresearch, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj openresearch czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do openresearch:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod openresearch. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://openresearch.ai/docs/sdk).
