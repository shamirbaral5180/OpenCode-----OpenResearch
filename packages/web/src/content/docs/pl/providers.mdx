---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w openresearch.
---

import config from "../../../../config.mjs"
export const console = config.console

openresearch używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji openresearch.
...
w `~/.local/share/openresearch/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim openresearch
config.
...
OpenResearch Zen to lista modeli dostarczonych przez zespół openresearch, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z openresearch. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenResearch Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenResearch Zen` i przejdź do [openresearch.ai/auth](https://openresearch.ai/zen).
...
Działa jak każdy inny dostawca w openresearch i jest całkowicie opcjonalny w użyciu.

---

## OpenResearch Go

OpenResearch Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół openresearch, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z openresearch.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenResearch Go` i przejdź do [openresearch.ai/auth](https://openresearch.ai/zen).
...
Aby używać Amazon Bedrock z openresearch:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania openresearch:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `openresearch.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w openresearch nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować openresearch do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod openresearch działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji openresearch.
...
openresearch używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować openresearch
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`openresearch.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [openresearch-gitlab-auth](https://www.npmjs.com/package/openresearch-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem openresearch:
...
Aby używać Google Vertex AI z openresearch:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania openresearch.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą openresearch, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [openresearch-helicone-session](https://github.com/H2Shami/openresearch-helicone-session), aby automatycznie rejestrować każdą konwersację openresearch jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja openresearch będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować openresearch tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować openresearch tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować openresearch tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem openresearch. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/openresearch).
:::
...
Aby korzystać z Ollama Cloud z openresearch:
...
3. Skopiuj klucz API do użycia w openresearch.
...
6. **Ważne**: Przed użyciem modeli chmurowych w openresearch musisz pobrać informacje o modelu lokalnie:
...
OpenResearch Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół openresearch. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OpenResearch Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OpenResearch Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API openresearch.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji openresearch.
...
5. Możesz je także dostosować za pomocą konfiguracji openresearch. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z openresearch:
...
Możesz także dostosować modele za pomocą konfiguracji openresearch. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z openresearch. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `openresearch.json` w katalogu projektu:
...
Pola `limit` pozwalają openresearch zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `openresearch auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację openresearch i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji openresearch.
