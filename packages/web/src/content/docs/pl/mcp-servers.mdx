...
Możesz dodać dodatkowe informacje do openresearch za pomocą _Model Context Protocol_, w skrócie MCP. openresearch obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [openresearch Config](https://openresearch.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/openresearch`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
openresearch automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, openresearch:
...
Jeśli serwer wymaga uwierzytelnienia, openresearch poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `openresearch mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji openresearch bezpieczne przechowa tokeny w `~/.local/share/openresearch/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w openresearch, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie openresearch, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć openresearch z kontem Sentry.
