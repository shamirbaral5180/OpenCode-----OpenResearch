...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. OpenResearch posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w OpenResearch.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OPENRESEARCH_EXPERIMENTAL_LSP_TOOL=true` (lub `OPENRESEARCH_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenResearch lub gdy zmienna środowiskowa `OPENRESEARCH_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu OpenResearch:
