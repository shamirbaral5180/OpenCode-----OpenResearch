---
title: Serwer
description: Interakcja z serwerem openresearch za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `openresearch serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient openresearch.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `openresearch.local` |
...
Ustaw `OPENRESEARCH_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `openresearch` lub ustaw `OPENRESEARCH_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `openresearch serve`, jak i `openresearch web`.
...
Kiedy uruchomisz `openresearch`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera openresearch do programowej interakcji z openresearch.
:::

Ta architektura umożliwia obsługę wielu klientów przez openresearch i programową interakcję z kodem otwartym.

Możesz uruchomić `openresearch serve`, aby uruchomić samodzielny serwer. Jeśli masz
openresearch TUI działa, `openresearch serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki openresearch [IDE](/docs/ide).
...
Serwer openresearch udostępnia następujące interfejsy API.
