...
代理技能让 OpenResearch 能够从你的仓库或主目录中发现可复用的指令。
...
OpenResearch 会搜索以下位置：

- 项目配置：`.openresearch/skills/<name>/SKILL.md`
- 全局配置：`~/.config/openresearch/skills/<name>/SKILL.md`
...
对于项目本地路径，OpenResearch 会从当前工作目录向上遍历，直到到达 git 工作树根目录。
在此过程中，它会加载 `.openresearch/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全局定义也会从 `~/.config/openresearch/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中加载。
...
创建 `.openresearch/skills/git-release/SKILL.md`，内容如下：
...
OpenResearch 会在 `skill` 工具描述中列出可用技能。
每个条目包含技能名称和描述：
...
在 `openresearch.json` 中使用基于模式的权限来控制代理可以访问哪些技能：
