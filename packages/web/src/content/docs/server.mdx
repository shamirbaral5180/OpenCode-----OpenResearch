---
title: Server
description: Interact with openresearch server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `openresearch serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an openresearch client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `openresearch.local` |
...
Set `OPENRESEARCH_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openresearch`, or set `OPENRESEARCH_SERVER_USERNAME` to override it. This applies to both `openresearch serve` and `openresearch web`.
...
When you run `openresearch` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the openresearch server to interact with openresearch programmatically.
:::

This architecture lets openresearch support multiple clients and allows you to interact with openresearch programmatically.

You can run `openresearch serve` to start a standalone server. If you have the
openresearch TUI running, `openresearch serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenResearch [IDE](/docs/ide) plugins.
...
The openresearch server exposes the following APIs.
