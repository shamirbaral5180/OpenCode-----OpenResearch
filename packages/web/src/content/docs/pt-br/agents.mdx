...
Existem dois tipos de agentes no openresearch; agentes primários e subagentes.
...
openresearch vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
OpenResearch vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

OpenResearch vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do OpenResearch, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `openresearch.json`:
...
- Global: `~/.config/openresearch/agents/`
- Por projeto: `.openresearch/agents/`
...
Se nenhuma temperatura for especificada, o openresearch usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do openresearch quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do openresearch usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenResearch Zen](/docs/zen), você usaria `openresearch/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `openresearch models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/openresearch).
:::
