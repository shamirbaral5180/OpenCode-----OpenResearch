...
Com o openresearch, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o openresearch usa nosso próprio tema `openresearch`.
...
O openresearch vem com vários temas embutidos.
...
- Querem que o openresearch corresponda à aparência do seu terminal
...
O openresearch suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/openresearch/themes/*.json` ou `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.openresearch/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.openresearch/themes/*.json`
