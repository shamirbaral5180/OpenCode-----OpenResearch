---
title: Plugins
description: Escreva seus próprios plugins para estender o openresearch.
---

Plugins permitem que você estenda o openresearch conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do openresearch.
...
- `.openresearch/plugins/` - Plugins em nível de projeto
- `~/.config/openresearch/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/openresearch/node_modules/`.
...
1. Configuração global (`~/.config/openresearch/openresearch.json`)
2. Configuração do projeto (`openresearch.json`)
3. Diretório de plugins global (`~/.config/openresearch/plugins/`)
4. Diretório de plugins do projeto (`.openresearch/plugins/`)
...
O openresearch executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do openresearch para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o openresearch.
...
:::note
Se você estiver usando o aplicativo desktop openresearch, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o openresearch de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao openresearch:
...
O helper `tool` cria uma ferramenta personalizada que o openresearch pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o openresearch junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://openresearch.ai/docs/sdk) para detalhes.
