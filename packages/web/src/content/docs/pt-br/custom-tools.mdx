...
description: Crie ferramentas que o LLM pode chamar no openresearch.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do openresearch, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.openresearch/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/openresearch/tools/`.
