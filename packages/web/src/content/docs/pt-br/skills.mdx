...
As habilidades do agente permitem que o openresearch descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O openresearch pesquisa nesses locais:

- Configuração do projeto: `.openresearch/skills/<name>/SKILL.md`
- Configuração global: `~/.config/openresearch/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o openresearch sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.openresearch/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/openresearch/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.openresearch/skills/git-release/SKILL.md` assim:
...
O openresearch lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `openresearch.json`:
...
**Para agentes embutidos** (em `openresearch.json`):
