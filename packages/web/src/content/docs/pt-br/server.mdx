---
title: Servidor
description: Interaja com o servidor openresearch via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `openresearch serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente openresearch pode usar.
...
```bash
openresearch serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `openresearch.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
openresearch serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENRESEARCH_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `openresearch`, ou defina `OPENRESEARCH_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `openresearch serve` quanto ao `openresearch web`.

```bash
OPENRESEARCH_SERVER_PASSWORD=your-password openresearch serve
```
...
Quando você executa `openresearch`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor openresearch para interagir com o openresearch programaticamente.
:::

Essa arquitetura permite que o openresearch suporte múltiplos clientes e permite que você interaja com o openresearch programaticamente.

Você pode executar `openresearch serve` para iniciar um servidor autônomo. Se você tiver o TUI do openresearch em execução, `openresearch serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do openresearch [IDE](/docs/ide).
...
O servidor openresearch expõe as seguintes APIs.
