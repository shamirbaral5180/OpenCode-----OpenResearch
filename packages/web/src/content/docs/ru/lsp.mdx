---
title: LSP-серверы
description: openresearch интегрируется с вашими серверами LSP.
---

openresearch может интегрироваться с серверами Language Server Protocol (LSP), чтобы использовать диагностику как обратную связь для агента.

---

## Встроенные

openresearch поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `OPENRESEARCH_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда LSP включен и openresearch открывает файл, он:
...
Вы можете включить и настроить серверы LSP через раздел `lsp` в конфигурации openresearch.
