...
Создайте `.openresearch/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию openresearch или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем openresearch [config](/docs/config):
...
- Глобальный: `~/.config/openresearch/commands/`
- Для каждого проекта: `.openresearch/commands/`
...
openresearch включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
