...
You can add external tools to OpenResearch using the _Model Context Protocol_, or MCP. OpenResearch supports both local and remote servers.
...
You can define MCP servers in your [OpenResearch Config](https://openresearch.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/openresearch` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OpenResearch automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenResearch will:
...
If the server requires authentication, OpenResearch will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `openresearch mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenResearch will store the tokens securely in `~/.local/share/openresearch/mcp-auth.json`.
...
Your MCPs are available as tools in OpenResearch, alongside built-in tools. So you can manage them through the OpenResearch config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenResearch to your Sentry account.
