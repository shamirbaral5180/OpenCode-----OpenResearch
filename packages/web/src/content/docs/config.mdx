---
title: Config
description: Using the OpenResearch JSON config.
---

You can configure OpenResearch using a JSON config file.

---

## Format

OpenResearch supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="openresearch.jsonc"
{
...
1. **Remote config** (from `.well-known/openresearch`) - organizational defaults
2. **Global config** (`~/.config/openresearch/openresearch.json`) - user preferences
3. **Custom config** (`OPENRESEARCH_CONFIG` env var) - custom overrides
4. **Project config** (`openresearch.json` in project) - project-specific settings
5. **`.openresearch` directories** - agents, commands, plugins
6. **Inline config** (`OPENRESEARCH_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/openresearch/` on macOS) - admin-controlled
...
:::note
The `.openresearch` and `~/.config/openresearch` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/openresearch` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/openresearch"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global OpenResearch config in `~/.config/openresearch/openresearch.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/openresearch/tui.json`.
...
Add `openresearch.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When OpenResearch starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `OPENRESEARCH_CONFIG` environment variable.

```bash
export OPENRESEARCH_CONFIG=/path/to/my/custom-config.json
openresearch run "Hello world"
```
...
Specify a custom config directory using the `OPENRESEARCH_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.openresearch` directory, and should
follow the same structure.

```bash
export OPENRESEARCH_CONFIG_DIR=/path/to/my/config-directory
openresearch run "Hello world"
```

The custom directory is loaded after the global config and `.openresearch` directories, so it **can override** their settings.
...
Drop an `openresearch.json` or `openresearch.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/openresearch/` |
| Linux    | `/etc/openresearch/`                         |
| Windows  | `%ProgramData%\openresearch`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, OpenResearch reads managed preferences from the `ai.openresearch.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

OpenResearch checks these paths:

1. `/Library/Managed Preferences/<user>/ai.openresearch.managed.plist`
2. `/Library/Managed Preferences/ai.openresearch.managed.plist`

The plist keys map directly to `openresearch.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.openresearch.managed` PayloadType. The OpenResearch config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.openresearch.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.openresearch.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.openresearch</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`openresearch.ai/config.json`**](https://openresearch.ai/config.json).

TUI config uses [**`openresearch.ai/tui.json`**](https://openresearch.ai/tui.json).
...
Use `OPENRESEARCH_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `openresearch.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `openresearch serve` and `openresearch web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your OpenResearch server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `openresearch.local`. Useful for running multiple instances on the same network.
...
If not specified, OpenResearch will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your OpenResearch config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, OpenResearch tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny OpenResearch actions on configured resources. Currently, policies can control which providers OpenResearch may use.
...
OpenResearch normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, OpenResearch omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/openresearch/agents/` or `.openresearch/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, OpenResearch will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`openresearch run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/openresearch/commands/` or `.openresearch/commands/`. [Learn more here](/docs/commands).
...
OpenResearch uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
OpenResearch will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, openresearch **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend OpenResearch with custom tools, hooks, and integrations.

Place plugin files in `.openresearch/plugins/` or `~/.config/openresearch/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="openresearch.json"
{
  "$schema": "https://openresearch.ai/config.json",
  "plugin": ["openresearch-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict OpenResearch to only use specific providers rather than disabling them one by one.
