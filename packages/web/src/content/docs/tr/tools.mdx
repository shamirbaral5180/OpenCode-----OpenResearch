...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. openresearch yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
openresearch içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `OPENRESEARCH_EXPERIMENTAL_LSP_TOOL=true` (veya `OPENRESEARCH_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca openresearch provider kullanılırken veya `OPENRESEARCH_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

openresearch başlatırken etkinleştirmek için:
