...
openresearch ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak openresearch kendi `openresearch` temasını kullanır.
...
openresearch birden fazla yerleşik temayla gelir.
...
- openresearch'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
openresearch, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/openresearch/themes/*.json` veya `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.openresearch/themes/*.json`
4. **Geçerli çalışma dizini** - `./.openresearch/themes/*.json`
