---
title: Sağlayıcılar
description: openresearch'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

openresearch, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı openresearch yapılandırmanızda yapılandırın.
...
`~/.local/share/openresearch/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları openresearch'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenResearch Zen, openresearch ekibi tarafından sağlanan modellerin bir listesidir.
openresearch ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenResearch Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, openresearch'u seçin ve [openresearch.ai/auth](https://openresearch.ai/auth)'ye gidin.
...
openresearch'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## OpenResearch Go

OpenResearch Go, openresearch ile iyi çalıştığı test edilmiş ve doğrulanmış, openresearch ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `OpenResearch Go`'yu seçin ve [openresearch.ai/auth](https://openresearch.ai/zen) adresine gidin.
...
Amazon Bedrock'u openresearch ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   openresearch'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `openresearch.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi openresearch'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
openresearch'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   openresearch'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   OpenResearch yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
openresearch, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. openresearch'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`openresearch.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [openresearch-gitlab-auth](https://www.npmjs.com/package/openresearch-gitlab-auth) ana sayfasında.

##### Yapılandırma

`openresearch.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi openresearch'la kullanmak için:
...
Google Vertex AI'yi openresearch ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları openresearch çalıştırılırken ayarlayın.
...
Helicone'un openresearch aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her openresearch görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [openresearch-helicone-session](https://github.com/H2Shami/openresearch-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her openresearch görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
openresearch'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla openresearch'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla openresearch'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini openresearch için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/openresearch)'a bakın.
:::
...
Ollama Cloud'u openresearch ile kullanmak için:
...
3. openresearch'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: openresearch'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenResearch Zen, openresearch ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>OpenResearch Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **OpenResearch Zen**'i arayın.

   ```txt
   /connect
   ```

3. openresearch API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   openresearch yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca openresearch yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi OpenResearch ile kullanmak için:
...
Modelleri openresearch yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı openresearch ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `openresearch.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları openresearch'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `openresearch auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için openresearch yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, openresearch yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
