...
description: OpenResearch, LSP sunucularınızla bütünleşir.
---

OpenResearch, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

OpenResearch, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`OPENRESEARCH_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve openresearch bir dosyayı açtığında:
...
LSP sunucularını, openresearch yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
