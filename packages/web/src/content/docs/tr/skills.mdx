...
Ajan becerileri, openresearch'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
openresearch su konumlari tarar:

- Proje konfigurasyonu: `.openresearch/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/openresearch/skills/<name>/SKILL.md`
...
Proje yerel yollarinda openresearch, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.openresearch/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/openresearch/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.openresearch/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
openresearch, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`openresearch.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`openresearch.json` icinde):
