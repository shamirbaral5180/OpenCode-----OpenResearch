...
OpenResearch Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.

Go, OpenResearch'daki diğer sağlayıcılar gibi çalışır. OpenResearch Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve OpenResearch'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OpenResearch Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

OpenResearch Go, OpenResearch'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OpenResearch Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `OpenResearch Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye OpenResearch Go'ya abone olabilir.
:::
...
OpenResearch Go, [OpenResearch](https://openresearch.ai) ve benzer türde istekler üreten diğer kodlama aracıları için tasarlanmıştır.
...
3. Yönlendirmeyi ve istem önbelleğe almayı optimize edebilmemiz için her konuşmada `x-openresearch-session` içinde değişmeyen bir oturum kimliği göndermelidir.

### Doğrulanmış İstemciler

OpenResearch'un yanı sıra aşağıdaki istemcilerin OpenResearch Go ile düzgün çalıştığı doğrulanmıştır.
Ancak gelecekte de çalışmaya devam edeceklerini garanti etmiyoruz.

| İstemci           | Oturum desteği                                                                                                                                                                                                                                               |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Hermes**        | [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) içeren derlemeler, ana ve yardımcı OpenResearch isteklerinde başlığı gönderir. Düzeltme v0.21.0'dan sonra birleştirilmiştir; dolayısıyla v0.21.0 sürümü tek başına bu düzeltmeyi içermez. |
...
| **ZCode**         | Go, ZCode'un yerel oturum başlığını tanır. [`x-openresearch-session` talebimiz](https://github.com/zai-org/feedback/issues/492) hâlâ açıktır ancak artık özellikle bu başlığın gönderilmesi gerekli değildir.                                                    |
| **Pi**            | Güncel derlemeler OpenResearch için oturum bilgilerini gönderir. Eski kurulumları güncelleyin.                                                                                                                                                                   |
...
| **Kilo Code CLI** | [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) içeren derlemeler OpenResearch oturum başlıklarını geri getirir. Bu düzeltme CLI'ı kapsar, VS Code uzantısını kapsamaz. Bkz. [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723).     |
...
OpenResearch yapılandırmanızdaki [model id](/docs/config/#models) formatı `openresearch-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `openresearch-go/kimi-k3` kullanmalısınız.
...
OpenResearch Go'yu şu amaçlarla oluşturduk:
...
4. OpenResearch ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
