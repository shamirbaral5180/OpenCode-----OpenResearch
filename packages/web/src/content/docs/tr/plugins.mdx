---
title: Eklentiler
description: OpenResearch'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek OpenResearch'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya OpenResearch'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.openresearch/plugins/` - Proje düzeyinde eklentiler
- `~/.config/openresearch/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/openresearch/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/openresearch/openresearch.json`)
2. Project config (`openresearch.json`)
3. Global eklenti dizini (`~/.config/openresearch/plugins/`)
4. Proje eklenti dizini (`.openresearch/plugins/`)
...
OpenResearch bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir OpenResearch SDK istemcisi.
...
OpenResearch'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
OpenResearch masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
OpenResearch'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca OpenResearch'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, OpenResearch'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra OpenResearch'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://openresearch.ai/docs/sdk) bakın.
