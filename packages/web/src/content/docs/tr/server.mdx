---
title: Sunucu
description: openresearch sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`openresearch serve` komutu, openresearch istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `openresearch.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OPENRESEARCH_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `openresearch` değeridir; değiştirmek isterseniz `OPENRESEARCH_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `openresearch serve` hem de `openresearch web` için geçerlidir.
...
`openresearch` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
openresearch ile programatik etkileşim için openresearch sunucusunu kullanın.
:::

Bu mimari, openresearch'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`openresearch serve` ile bağımsız bir sunucu başlatabilirsiniz. openresearch TUI açıksa, `openresearch serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum openresearch [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
openresearch sunucusu aşağıdaki API'leri sunar.
