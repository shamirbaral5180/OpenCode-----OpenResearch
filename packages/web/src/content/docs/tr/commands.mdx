...
`.openresearch/commands/test.md` oluştur:
...
openresearch işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

openresearch [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/openresearch/commands/`
- Proje başına: `.openresearch/commands/`
...
openresearch, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
