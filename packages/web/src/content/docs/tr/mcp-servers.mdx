...
_Model Bağlam Protokolü_ veya MCP kullanarak openresearch'a harici araçlar ekleyebilirsiniz. openresearch hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [openresearch Config](https://openresearch.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/openresearch` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
openresearch, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde openresearch şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa openresearch, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `openresearch mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından openresearch, belirteçleri `~/.local/share/openresearch/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra openresearch'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi openresearch yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve openresearch'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
