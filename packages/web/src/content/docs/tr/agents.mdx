...
openresearch'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
openresearch, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
OpenResearch, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

OpenResearch iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `openresearch.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.openresearch/agents/`
...
Sıcaklık belirtilmezse openresearch modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global openresearch yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
openresearch yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [OpenResearch Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `openresearch/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `openresearch models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/openresearch).
:::
