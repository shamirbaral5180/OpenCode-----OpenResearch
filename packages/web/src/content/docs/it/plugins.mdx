---
title: Plugin
description: Scrivi plugin per estendere OpenResearch.
---

I plugin ti permettono di estendere OpenResearch agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di OpenResearch.
...
- `.openresearch/plugins/` - plugin a livello progetto
- `~/.config/openresearch/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/openresearch/node_modules/`.
...
1. Config globale (`~/.config/openresearch/openresearch.json`)
2. Config di progetto (`openresearch.json`)
3. Directory plugin globale (`~/.config/openresearch/plugins/`)
4. Directory plugin di progetto (`.openresearch/plugins/`)
...
OpenResearch esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di openresearch per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere openresearch.
...
:::note
Se usi l'app desktop di OpenResearch, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a openresearch di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a openresearch:
...
L'helper `tool` crea uno strumento personalizzato che openresearch può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in openresearch insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://openresearch.ai/docs/sdk) per i dettagli.
