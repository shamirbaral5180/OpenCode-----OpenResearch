...
Con OpenResearch puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, OpenResearch usa il tema `openresearch`.
...
OpenResearch include diversi temi integrati.
...
- Vuole che OpenResearch corrisponda all'aspetto del terminale
...
OpenResearch supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/openresearch/themes/*.json` o `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.openresearch/themes/*.json`
4. **Directory di lavoro corrente** - in `./.openresearch/themes/*.json`
