...
Puoi aggiungere strumenti esterni a OpenResearch usando il _Model Context Protocol_, o MCP. OpenResearch supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [OpenResearch Config](https://openresearch.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/openresearch`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
OpenResearch gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, OpenResearch:
...
Se il server richiede autenticazione, OpenResearch ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `openresearch mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, OpenResearch memorizzera' i token in modo sicuro in `~/.local/share/openresearch/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in OpenResearch insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione OpenResearch come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare OpenResearch al tuo account Sentry.
