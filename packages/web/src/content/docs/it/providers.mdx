---
title: Provider
description: Usa qualsiasi provider LLM in OpenResearch.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenResearch usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di OpenResearch.
...
configurazione di OpenResearch.
...
OpenResearch Zen e una lista di modelli forniti dal team OpenResearch, testati e
verificati per funzionare bene con OpenResearch. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da OpenResearch Zen.
:::

1. Esegui `/connect` nella TUI, seleziona openresearch e vai su [openresearch.ai/auth](https://openresearch.ai/auth).
...
Funziona come qualsiasi altro provider in OpenResearch ed e totalmente opzionale.
...
Per usare Amazon Bedrock con OpenResearch:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui openresearch:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `openresearch.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in OpenResearch non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare openresearch per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché openresearch funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di openresearch.
...
2. Scegli il tuo metodo di autenticazione:
   ...
   3. Nome: `OpenResearch`, Scopes: `api`
...
salvare il token nello storage di auth di openresearch.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
OpenResearch usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare OpenResearch
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `openresearch.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [openresearch-gitlab-auth](https://www.npmjs.com/package/openresearch-gitlab-auth).

##### Configurazione

Personalizza tramite `openresearch.json`:
...
Per usare il tuo abbonamento GitHub Copilot con openresearch:
...
Per usare Google Vertex AI con OpenResearch:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui openresearch.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da openresearch, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [openresearch-helicone-session](https://github.com/H2Shami/openresearch-helicone-session) per loggare automaticamente ogni conversazione di OpenResearch come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di OpenResearch elencata come sessione separata.
...
Puoi configurare openresearch per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare openresearch per usare modelli locali tramite LM Studio.
...
Puoi configurare openresearch per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per OpenResearch. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/openresearch) per dettagli.
:::
...
Per usare Ollama Cloud con OpenResearch:
...
3. Copia la chiave API da usare in OpenResearch.
...
6. **Importante**: Prima di usare modelli cloud in OpenResearch, devi scaricare le informazioni del modello localmente:
...
OpenResearch Zen è una lista di modelli testati e verificati forniti dal team OpenResearch. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>OpenResearch Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **OpenResearch Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di OpenResearch.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di openresearch.
...
5. Puoi anche personalizzarli tramite la configurazione di openresearch. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con OpenResearch:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di openresearch. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con openresearch. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `openresearch.json` nella directory del progetto:
...
I campi `limit` permettono a OpenResearch di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `openresearch auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di openresearch e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config openresearch.
