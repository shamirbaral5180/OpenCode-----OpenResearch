---
title: Server
description: Interagisci con il server openresearch via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `openresearch serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client openresearch.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `openresearch.local` |
...
Imposta `OPENRESEARCH_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `openresearch`, oppure imposta `OPENRESEARCH_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `openresearch serve` sia per `openresearch web`.
...
Quando esegui `openresearch` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server openresearch per interagire con openresearch in modo programmatico.
:::

Questa architettura permette a openresearch di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `openresearch serve` per avviare un server standalone. Se la TUI di openresearch e gia in esecuzione, `openresearch serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di OpenResearch.
...
Il server openresearch espone le seguenti API.
