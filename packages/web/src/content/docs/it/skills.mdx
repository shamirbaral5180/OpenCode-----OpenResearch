...
Le skill degli agenti permettono a OpenResearch di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenResearch cerca in queste posizioni:

- Config di progetto: `.openresearch/skills/<name>/SKILL.md`
- Config globale: `~/.config/openresearch/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenResearch risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.openresearch/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/openresearch/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.openresearch/skills/git-release/SKILL.md` cosi':
...
OpenResearch elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `openresearch.json`:
...
**Per agenti integrati** (in `openresearch.json`):
