---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OpenResearch.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenResearch fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OpenResearch avvia la TUI per la directory corrente.
...
Quando usi la TUI di OpenResearch, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OpenResearch. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OpenResearch. _Alias_: `/quit`, `/q`
...
Questo è separato da `openresearch.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo OpenResearch attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `openresearch.default`.
