---
title: Policies
description: Control which configured resources OpenResearch may use.
---

Policies control whether OpenResearch may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `openresearch.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether OpenResearch may use a resource such as an LLM provider.
...
OpenResearch currently supports one policy action:
