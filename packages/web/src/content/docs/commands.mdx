...
Create `.openresearch/commands/test.md`:
...
You can add custom commands through the OpenResearch config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OpenResearch [config](/docs/config):
...
- Per-project: `.openresearch/commands/`
...
openresearch includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
