---
title: LSP Servers
description: OpenResearch integrates with your LSP servers.
---

OpenResearch can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

OpenResearch comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `OPENRESEARCH_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and openresearch opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your openresearch config.
...
```json title="openresearch.json" {5-8}
{
  "$schema": "https://openresearch.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="openresearch.json" {5-13}
{
  "$schema": "https://openresearch.ai/config.json",
  "lsp": {
    "custom-lsp": {
