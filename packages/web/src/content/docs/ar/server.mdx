---
title: الخادم
description: تفاعل مع خادم openresearch عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `openresearch serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل openresearch استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `openresearch.local` |
...
عيّن `OPENRESEARCH_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `openresearch`، أو عيّن `OPENRESEARCH_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `openresearch serve` و `openresearch web`.
...
عند تشغيل `openresearch` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم openresearch للتفاعل مع openresearch برمجيا.
:::

تتيح هذه البنية لـ openresearch دعم عدة عملاء وتمكّنك من التفاعل مع openresearch برمجيا.

يمكنك تشغيل `openresearch serve` لبدء خادم مستقل. إذا كانت واجهة openresearch في terminal (TUI)
قيد التشغيل، فسيبدأ `openresearch serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات OpenResearch لـ [IDE](/docs/ide).
...
يعرض خادم openresearch واجهات API التالية.
