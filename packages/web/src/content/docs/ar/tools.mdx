...
تتيح الأدوات لـ LLM تنفيذ إجراءات داخل قاعدة الشفرة الخاصة بك. يأتي OpenResearch مع مجموعة من الأدوات المدمجة، لكن يمكنك توسيعه عبر [أدوات مخصصة](/docs/custom-tools) أو [خوادم MCP](/docs/mcp-servers).
...
فيما يلي جميع الأدوات المدمجة المتاحة في OpenResearch.
...
:::note
هذه الأداة متاحة فقط عند ضبط `OPENRESEARCH_EXPERIMENTAL_LSP_TOOL=true` (أو `OPENRESEARCH_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenResearch أو عند ضبط متغير البيئة `OPENRESEARCH_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).

للتفعيل عند تشغيل OpenResearch:
