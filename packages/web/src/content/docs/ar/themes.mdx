...
مع OpenResearch يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenResearch سمتنا `openresearch`.
...
يأتي OpenResearch مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenResearch مظهر terminal لديهم
...
يدعم OpenResearch نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/openresearch/themes/*.json` أو `$XDG_CONFIG_HOME/openresearch/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.openresearch/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.openresearch/themes/*.json`
