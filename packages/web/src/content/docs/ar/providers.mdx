---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenResearch.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenResearch ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenResearch.
...
في `~/.local/share/openresearch/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenResearch.
...
OpenResearch Zen هي قائمة نماذج يوفّرها فريق OpenResearch وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenResearch. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenResearch Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر openresearch، ثم انتقل إلى [openresearch.ai/auth](https://openresearch.ai/auth).
...
يعمل مثل أي مزوّد آخر في OpenResearch واستخدامه اختياري بالكامل.

---

## OpenResearch Go

OpenResearch Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق OpenResearch، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenResearch.

1. شغّل الأمر `/connect` في TUI، واختر `OpenResearch Go`، ثم انتقل إلى [openresearch.ai/auth](https://openresearch.ai/zen).
...
لاستخدام Amazon Bedrock مع OpenResearch:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل openresearch:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `openresearch.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenResearch غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين openresearch لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل openresearch بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات openresearch.
...
تخزين الرمز في مساحة تخزين مصادقة openresearch.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم OpenResearch نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenResearch
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`openresearch.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [openresearch-gitlab-auth](https://www.npmjs.com/package/openresearch-gitlab-auth).

##### التهيئة

خصّص عبر `openresearch.json`:
...
لاستخدام اشتراك GitHub Copilot مع openresearch:
...
لاستخدام Google Vertex AI مع OpenResearch:
...
2. عيّن متغيرات البيئة المطلوبة:
   ...
   عيّنها أثناء تشغيل openresearch.
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر openresearch، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [openresearch-helicone-session](https://github.com/H2Shami/openresearch-helicone-session) لتسجيل كل محادثة OpenResearch تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenResearch مدرجة كجلسة مستقلة.
...
يمكنك ضبط openresearch لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط openresearch لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط openresearch لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenResearch. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/openresearch) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenResearch:
...
3. انسخ مفتاح API لاستخدامه في OpenResearch.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenResearch، يجب سحب معلومات النموذج محليا:
...
OpenResearch Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenResearch. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenResearch Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenResearch Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenResearch.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات openresearch.
...
5. يمكنك أيضا تخصيصها عبر إعدادات openresearch. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع OpenResearch:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات openresearch. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع openresearch. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `openresearch.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ OpenResearch بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `openresearch auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات openresearch وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات openresearch.
