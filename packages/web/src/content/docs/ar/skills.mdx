...
تتيح مهارات الوكيل لـ OpenResearch اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث OpenResearch في هذه المواقع:

- إعدادات المشروع: `.openresearch/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/openresearch/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج OpenResearch صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.openresearch/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/openresearch/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.openresearch/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض OpenResearch المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `openresearch.json`:
...
**للوكلاء المدمجين** (في `openresearch.json`):
