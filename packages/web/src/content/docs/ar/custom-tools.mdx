...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في openresearch.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في openresearch مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.openresearch/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/openresearch/tools/`.
