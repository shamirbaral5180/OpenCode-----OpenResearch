---
title: TUI
description: استخدام واجهة المستخدم TUI في OpenResearch.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر OpenResearch واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل OpenResearch إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة OpenResearch في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى OpenResearch. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من OpenResearch. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `openresearch.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار OpenResearch بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `openresearch.default`.
