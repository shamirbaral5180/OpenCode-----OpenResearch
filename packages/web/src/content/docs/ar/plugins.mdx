---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع OpenResearch.
---

تتيح لك الإضافات توسيع OpenResearch عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ OpenResearch.
...
- `.openresearch/plugins/` - إضافات على مستوى المشروع
- `~/.config/openresearch/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/openresearch/node_modules/`.
...
1. إعدادات عامة (`~/.config/openresearch/openresearch.json`)
2. إعدادات المشروع (`openresearch.json`)
3. دليل الإضافات العام (`~/.config/openresearch/plugins/`)
4. دليل إضافات المشروع (`.openresearch/plugins/`)
...
يشغّل OpenResearch الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ openresearch للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع openresearch.
...
:::note
إذا كنت تستخدم تطبيق OpenResearch لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع openresearch من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى openresearch:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ openresearch استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ openresearch إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://openresearch.ai/docs/sdk) للتفاصيل.
