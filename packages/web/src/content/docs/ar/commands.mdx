...
أنشئ الملف `.openresearch/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OpenResearch أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OpenResearch:
...
- على مستوى النظام: `~/.config/openresearch/commands/`
- لكل مشروع: `.openresearch/commands/`
...
يتضمن openresearch عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
